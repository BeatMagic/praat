...
/*
 ...
 this is necessary because these functions have to be used in an editor
 window, where they have to work in the background, because they are not
 explicly called by a user action.

 Postcondition:
 - If a tryToCompute<Analysis>() function fails, the <Analysis> should be
 null; this is how tryToCompute<Analysis>() signals failure.
 */
static autoSound extractSound (SoundAnalysisArea me, double tmin, double tmax) {
	...
	if (my longSound ()) {
		...
		sound = Sound_extractPart (my sound (), tmin, tmax,
...
autoSpectrogram Sound_to_CQT_Ultra_Fast (Sound me, double fmin, double fmax,
	...
	try {
		...
		const double minimumTimeStep2 =
		        effectiveTimeWidth / maximumTimeOversampling;
		const double timeStep = (minimumTimeStep1 > minimumTimeStep2)
		                                ? minimumTimeStep1
		                                : minimumTimeStep2;
		const double physicalDuration = my dx * my nx;
		const double maxWindowLength = Q / fmin;

		if (maxWindowLength > physicalDuration)
			Melder_throw (U"Your sound is too short for the requested "
			              U"frequency range.");

		const integer numberOfTimes =
		        1 +
		        Melder_ifloor ((physicalDuration - maxWindowLength) / timeStep);
		...
		for (integer validIdx = 1; validIdx <= validFreqCount; validIdx++) {
			...
			for (integer isamp = 0; isamp < nsamp_window; isamp++) {
				...
				case kSound_to_Spectrogram_windowShape::HAMMING:
					windowValue = 0.54 - 0.46 * cos (2.0 * NUMpi * phase);
					break;
				case kSound_to_Spectrogram_windowShape::HANNING:
					windowValue = 0.5 * (1.0 - cos (2.0 * NUMpi * phase));
					break;
				case kSound_to_Spectrogram_windowShape::GAUSSIAN: {
					const double center = 0.5 * (nsamp_window + 1);
					const double sigma = nsamp_window / 6.0;
					const double x = isamp + 1 - center;
					windowValue = exp (-0.5 * x * x / (sigma * sigma));
					break;
				}
				default:
					windowValue = 1.0;
				...
				sinTable[validIdx][isamp + 1] = sin (omega);
			}
		...
		for (integer timeBlock = 0; timeBlock < numberOfTimes;
			...
			for (integer validIdx = 1; validIdx <= validFreqCount; validIdx++) {
				...
				for (integer iframe = timeStart; iframe <= timeEnd;
					...
					const integer startSample = centerSample - halfWindow;
					const integer endSample = centerSample + halfWindow - 1;
					...
					for (integer i = startSample; i <= endSample; i += 4) {
						if (my ny == 1) {
							double sample = my z[1][i];
							energySum += sample * sample;
						} else {
							for (integer channel = 1; channel <= my ny;
							        channel++) {
								double sample = my z[channel][i];
								energySum += sample * sample;
							}
						}
					}
...
static double getBinsPerOctaveFromEnum (kCQT_binsPerOctave binsEnum) {
	switch (binsEnum) {
	case kCQT_binsPerOctave::BINS_12:
		return 12.0;
	case kCQT_binsPerOctave::BINS_24:
		return 24.0;
	case kCQT_binsPerOctave::BINS_36:
		return 36.0;
	case kCQT_binsPerOctave::BINS_48:
		return 48.0;
	default:
		return 36.0;
	}
...
// Functions for logarithmic frequency scale in CQT mode
static double frequencyToLogPosition (
        double frequency, double fmin, double fmax) {
	if (frequency <= 0.0 || fmin <= 0.0 || fmax <= fmin) return 0.0;
	Melder_clip (fmin, &frequency, fmax);
	return log (frequency / fmin) / log (fmax / fmin);
}

static double logPositionToFrequency (
        double logPosition, double fmin, double fmax) {
	if (fmin <= 0.0 || fmax <= fmin) return fmin;
	Melder_clip (0.0, &logPosition, 1.0);
	return fmin * pow (fmax / fmin, logPosition);
}

// MIDI note conversion functions for CQT
...
static conststring32 midiNoteToNoteName (
        integer midiNote, kSoundAnalysisArea_pitchGrid_key key) {
	// Note names in C major (natural notes) - using Unicode sharp (♯) and flat
	// (♭) symbols
	static const conststring32 cMajorNotes[] = {U"C", U"C♯", U"D", U"D♯", U"E",
	        U"F", U"F♯", U"G", U"G♯", U"A", U"A♯", U"B"};
...
static conststring32 midiNoteToSolfege (
        integer midiNote, kSoundAnalysisArea_pitchGrid_key key) {
	// Standard solfege names (using standard ASCII characters for
	// compatibility)
	static const conststring32 standardSolfege[] = {U"do", U"di", U"re", U"ri",
	        U"mi", U"fa", U"fi", U"sol", U"si", U"la", U"li", U"ti"};
	...
	// Fixed Do is just Movable Do in C major, so we use the same logic for all
	// cases
	integer scaleDegree;
	switch (key) {
	case kSoundAnalysisArea_pitchGrid_key::C_MAJOR:
		// C major: C=do, C♯=di, D=re, D♯=ri, E=mi, F=fa, F♯=fi, G=sol, G♯=si,
		// A=la, A♯=li, B=ti
		scaleDegree = noteInOctave;
		break;
	case kSoundAnalysisArea_pitchGrid_key::G_MAJOR:
		// G major: G=do, G♯=di, A=re, A♯=ri, B=mi, C=fa, C♯=fi, D=sol, D♯=si,
		// E=la, F=li, F♯=ti
		if (noteInOctave == 6)
			scaleDegree = 6;   // F♯ -> ti
		else if (noteInOctave >= 7)
			scaleDegree = noteInOctave - 7;   // G=0, G♯=1, A=2, A♯=3, B=4, C=5,
			                                  // C♯=6, D=7, D♯=8, E=9, F=10
		else
			scaleDegree = noteInOctave +
			              5;   // C=5, C♯=6, D=7, D♯=8, E=9, F=10, F♯=11
		break;
	case kSoundAnalysisArea_pitchGrid_key::D_MAJOR:
		// D major: D=do, D♯=di, E=re, F=ri, F♯=mi, G=fa, G♯=fi, A=sol, A♯=si,
		// B=la, C=li, C♯=ti
		if (noteInOctave >= 2)
			scaleDegree =
			        noteInOctave - 2;   // D=0, D♯=1, E=2, F=3, F♯=4, G=5, G♯=6,
			                            // A=7, A♯=8, B=9, C=10, C♯=11
		else
			scaleDegree = noteInOctave + 10;   // C=10, C♯=11
		break;
	case kSoundAnalysisArea_pitchGrid_key::A_MAJOR:
		// A major: A=do, A♯=di, B=re, C=ri, C♯=mi, D=fa, D♯=fi, E=sol, F=si,
		// F♯=la, G=li, G♯=ti
		if (noteInOctave >= 9)
			scaleDegree = noteInOctave - 9;   // A=0, A♯=1, B=2
		else
			scaleDegree =
			        noteInOctave +
			        3;   // C=3, C♯=4, D=5, D♯=6, E=7, F=8, F♯=9, G=10, G♯=11
		break;
	case kSoundAnalysisArea_pitchGrid_key::E_MAJOR:
		// E major: E=do, F=di, F♯=re, G=ri, G♯=mi, A=fa, A♯=fi, B=sol, C=si,
		// C♯=la, D=li, D♯=ti
		if (noteInOctave >= 4)
			scaleDegree = noteInOctave - 4;   // E=0, F=1, F♯=2, G=3, G♯=4
		else
			scaleDegree = noteInOctave +
			              8;   // A=8, A♯=9, B=10, C=11, C♯=0, D=1, D♯=2
		break;
	case kSoundAnalysisArea_pitchGrid_key::B_MAJOR:
		// B major: B=do, C=di, C♯=re, D=ri, D♯=mi, E=fa, F=fi, F♯=sol, G=si,
		// G♯=la, A=li, A♯=ti
		if (noteInOctave >= 11)
			scaleDegree = noteInOctave - 11;   // B=0
		else
			scaleDegree = noteInOctave + 1;   // C=1, C♯=2, D=3, D♯=4, E=5, F=6,
			                                  // F♯=7, G=8, G♯=9, A=10, A♯=11
		break;
	case kSoundAnalysisArea_pitchGrid_key::F_SHARP_MAJOR:
		// F♯ major: F♯=do, G=di, G♯=re, A=ri, A♯=mi, B=fa, C=fi, C♯=sol, D=si,
		// D♯=la, E=li, F=ti
		if (noteInOctave >= 6)
			scaleDegree = noteInOctave - 6;   // F♯=0, G=1, G♯=2, A=3, A♯=4, B=5
		else
			scaleDegree =
			        noteInOctave + 6;   // C=6, C♯=7, D=8, D♯=9, E=10, F=11
		break;
	case kSoundAnalysisArea_pitchGrid_key::C_SHARP_MAJOR:
		// C♯ major: C♯=do, D=di, D♯=re, E=ri, F=mi, F♯=fa, G=fi, G♯=sol, A=si,
		// A♯=la, B=li, C=ti
		if (noteInOctave >= 1)
			scaleDegree =
			        noteInOctave - 1;   // C♯=0, D=1, D♯=2, E=3, F=4, F♯=5, G=6,
			                            // G♯=7, A=8, A♯=9, B=10
		else
			scaleDegree = noteInOctave + 11;   // C=11
		break;
	case kSoundAnalysisArea_pitchGrid_key::F_MAJOR:
		// F major: F=do, F♯=di, G=re, G♯=ri, A=mi, B♭=fa, B=fi, C=sol, C♯=si,
		// D=la, D♯=li, E=ti
		if (noteInOctave >= 5)
			scaleDegree = noteInOctave - 5;   // F=0, F♯=1, G=2, G♯=3, A=4
		else
			scaleDegree = noteInOctave +
			              7;   // B♭=7, B=8, C=9, C♯=10, D=11, D♯=0, E=1
		break;
	case kSoundAnalysisArea_pitchGrid_key::B_FLAT_MAJOR:
		// B♭ major: B♭=do, B=di, C=re, C♯=ri, D=mi, E♭=fa, E=fi, F=sol, F♯=si,
		// G=la, G♯=li, A=ti
		if (noteInOctave >= 10)
			scaleDegree = noteInOctave - 10;   // B♭=0, B=1
		else
			scaleDegree = noteInOctave + 2;   // C=2, C♯=3, D=4, E♭=5, E=6, F=7,
			                                  // F♯=8, G=9, G♯=10, A=11
		break;
	case kSoundAnalysisArea_pitchGrid_key::E_FLAT_MAJOR:
		// E♭ major: E♭=do, E=di, F=re, F♯=ri, G=mi, A♭=fa, A=fi, B♭=sol, B=si,
		// C=la, C♯=li, D=ti
		if (noteInOctave >= 3)
			scaleDegree = noteInOctave - 3;   // E♭=0, E=1, F=2
		else
			scaleDegree = noteInOctave +
			              9;   // A♭=9, A=10, B♭=11, B=0, C=1, C♯=2, D=3
		break;
	case kSoundAnalysisArea_pitchGrid_key::A_FLAT_MAJOR:
		// A♭ major: A♭=do, A=di, B♭=re, B=ri, C=mi, D♭=fa, D=fi, E♭=sol, E=si,
		// F=la, F♯=li, G=ti
		if (noteInOctave >= 8)
			scaleDegree = noteInOctave - 8;   // A♭=0, A=1, B♭=2, B=3
		else
			scaleDegree = noteInOctave +
			              4;   // C=4, D♭=5, D=6, E♭=7, E=8, F=9, F♯=10, G=11
		break;
	default:
		scaleDegree = noteInOctave;
		break;
	...
	if (my instancePref_cqt_show ()) {
		...
		try {
			...
			my d_spectrogram = Sound_to_Spectrogram (sound.get (),
			        ...
			        (my endWindow () - my startWindow ()) /
			                my instancePref_spectrogram_timeSteps (),
			        my instancePref_spectrogram_viewTo () /
			                my instancePref_spectrogram_frequencySteps (),
...
static void tryToComputePitch (SoundAnalysisArea me) {
	autoMelderProgressOff progress;
	const double margin = (my instancePref_pitch_veryAccurate ()
	                               ? 3.0 / my instancePref_pitch_floor ()
	                               : 1.5 / my instancePref_pitch_floor ());
	try {
		autoSound sound = extractSound (
		        me, my startWindow () - margin, my endWindow () + margin);
		const double pitchTimeStep =
		        (my instancePref_timeStepStrategy () ==
		                                kSoundAnalysisArea_timeStepStrategy::
		                                        FIXED_
		                        ? my instancePref_fixedTimeStep ()
		                : my instancePref_timeStepStrategy () ==
		                                kSoundAnalysisArea_timeStepStrategy::
		                                        VIEW_DEPENDENT
		                        ? (my endWindow () - my startWindow ()) /
		                                  my instancePref_numberOfTimeStepsPerView ()
		                        : 0.0   // the default: determined by pitch
		                                // floor
		        );
		my d_pitch = Sound_to_Pitch_any (sound.get (), pitchTimeStep,
		        my instancePref_pitch_floor (),
		        my instancePref_pitch_method () ==
		                        kSoundAnalysisArea_pitch_analysisMethod::
		                                AUTOCORRELATION
		                ? 3.0
		                : 1.0,
		        my instancePref_pitch_maximumNumberOfCandidates (),
		        ((int) my instancePref_pitch_method () - 1) * 2 +
		                my instancePref_pitch_veryAccurate (),
		        my instancePref_pitch_silenceThreshold (),
		        my instancePref_pitch_voicingThreshold (),
		        my instancePref_pitch_octaveCost (),
		        my instancePref_pitch_octaveJumpCost (),
		        my instancePref_pitch_voicedUnvoicedCost (),
		        my instancePref_pitch_ceiling ());
...
static void tryToComputeIntensity (SoundAnalysisArea me) {
	...
	try {
		autoSound sound = extractSound (
		        me, my startWindow () - margin, my endWindow () + margin);
		my d_intensity = Sound_to_Intensity (sound.get (),
		        my instancePref_pitch_floor (),
		        my endWindow () - my startWindow () >
		                        my instancePref_longestAnalysis ()
		                ? (my endWindow () - my startWindow ()) / 100
		                : 0.0,
...
static void tryToComputeFormants (SoundAnalysisArea me) {
	...
	try {
		        ...
		        (my endWindow () - my startWindow () >
		                        ...
		                        ? extractSound (me,
		                                  ...
		                                  0.5 * (my startWindow () +
		                                                ...
		                                                my instancePref_longestAnalysis ()) +
		                                          margin)
		                        : extractSound (me, my startWindow () - margin,
		                                  my endWindow () + margin));
		...
		my d_formant = Sound_to_Formant_any (sound.get (), formantTimeStep,
		        Melder_iround (
		                my instancePref_formant_numberOfFormants () * 2.0),
		        my instancePref_formant_ceiling (),
		        my instancePref_formant_windowLength (),
		        (int) my instancePref_formant_method (),
		        my instancePref_formant_preemphasisFrom (), 50.0);
...
static void tryToComputePulses (SoundAnalysisArea me) {
	if (!my d_pitch) tryToComputePitch (me);
	if (my d_pitch) {
		autoMelderProgressOff progress;
		try {
			autoSound sound =
			        extractSound (me, my startWindow (), my endWindow ());
			my d_pulses = Sound_Pitch_to_PointProcess_cc (
			        sound.get (), my d_pitch.get ());
		} catch (MelderError) {
...
/*
 ...
 The "try" again means that any exceptions that are generated, will be
 ignored; failure is again signalled by the Analysis being null. The "have"
 means that these functions do nothing if the Analysis already exists, but
...
/*
 ...
 These can be used for querying, extracting, and drawing to the Picture
 window, because those require the user to be aware of the need to have a
 visible Analysis. They cannot be used when drawing into the analysis part of
 the editor window, because of the low degree of explicitness of the need to
 draw an Analysis there.
 */
void SoundAnalysisArea_haveVisibleSpectrogram (SoundAnalysisArea me) {
	if (!my instancePref_spectrogram_show ())
		Melder_throw (U"No spectrogram is visible.\nFirst choose \"Show "
		              U"spectrogram\" from the Spectrogram menu.");
	tryToHaveSpectrogram (me);
	if (!my d_spectrogram)
		Melder_throw (
		        U"The spectrogram is not defined at the edge of the sound.");
}
void SoundAnalysisArea_haveVisiblePitch (SoundAnalysisArea me) {
	if (!my instancePref_pitch_show ())
		Melder_throw (U"No pitch contour is visible.\nFirst choose \"Show "
		              U"pitch\" from the Pitch menu.");
	tryToHavePitch (me);
	if (!my d_pitch)
		Melder_throw (
		        U"The pitch contour is not defined at the edge of the sound.");
}
void SoundAnalysisArea_haveVisibleIntensity (SoundAnalysisArea me) {
	if (!my instancePref_intensity_show ())
		Melder_throw (U"No intensity contour is visible.\nFirst choose \"Show "
		              U"intensity\" from the Intensity menu.");
	tryToHaveIntensity (me);
	if (!my d_intensity)
		Melder_throw (U"The intensity curve is not defined at the edge of the "
		              U"sound.");
}
void SoundAnalysisArea_haveVisibleFormants (SoundAnalysisArea me) {
	if (!my instancePref_formant_show ())
		Melder_throw (U"No formant contour is visible.\nFirst choose \"Show "
		              U"formants\" from the Formants menu.");
	tryToHaveFormants (me);
	if (!my d_formant)
		Melder_throw (
		        U"The formants are not defined at the edge of the sound.");
}
...
bool structSoundAnalysisArea ::v_mouse (GuiDrawingArea_MouseEvent event,
        double x_world, double localY_fraction) {
	if (event->isClick ()) {
		if (our instancePref_pitch_show ()) {
			if (x_world >= our endWindow () && localY_fraction > 0.96 &&
			        localY_fraction <= 1.00) {
				our setInstancePref_pitch_ceiling (
				        our instancePref_pitch_ceiling () * 1.26);
				our d_pitch.reset ();
				our d_intensity.reset ();
				our d_pulses.reset ();
				return FunctionEditor_UPDATE_NEEDED;
			}
			if (x_world >= our endWindow () && localY_fraction > 0.92 &&
			        localY_fraction <= 0.96) {
				our setInstancePref_pitch_ceiling (
				        our instancePref_pitch_ceiling () / 1.26);
				our d_pitch.reset ();
				our d_intensity.reset ();
	...
	if (our isMoveWhileButtonDown && x_world > our startWindow () &&
		...
		if (our instancePref_cqt_show () &&
			...
			// @lixu 临时关闭计算-NO
			our d_spectrogram_cursor = logPositionToFrequency (localY_fraction,
			        our instancePref_spectrogram_viewFrom (),
			        our instancePref_spectrogram_viewTo ());
		} else {
			// Use linear scale for regular spectrogram
			our d_spectrogram_cursor =
			        localY_fraction * our instancePref_spectrogram_viewTo () +
			        (1.0 - localY_fraction) *
			                our instancePref_spectrogram_viewFrom ();
		}
	}
	...
	if (x_world > our startWindow () && x_world < our endWindow ()) {
		const double pitchFloor_hidden_semitones440 =
		        Function_convertStandardToSpecialUnit (
		                Thing_dummyObject (Pitch),
		                our instancePref_pitch_floor (), Pitch_LEVEL_FREQUENCY,
		                (int) kPitch_unit::SEMITONES_440);
		const double pitchCeiling_hidden_semitones440 =
		        Function_convertStandardToSpecialUnit (
		                Thing_dummyObject (Pitch),
		                our instancePref_pitch_ceiling (),
		                Pitch_LEVEL_FREQUENCY,
		                (int) kPitch_unit::SEMITONES_440);
		const double pitchFloor_overt_semitones440 =
		        Function_convertToNonlogarithmic (Thing_dummyObject (Pitch),
		                pitchFloor_hidden_semitones440, Pitch_LEVEL_FREQUENCY,
		                (int) kPitch_unit::SEMITONES_440);
		const double pitchCeiling_overt_semitones440 =
		        Function_convertToNonlogarithmic (Thing_dummyObject (Pitch),
		                pitchCeiling_hidden_semitones440, Pitch_LEVEL_FREQUENCY,
		                (int) kPitch_unit::SEMITONES_440);
		const double pitchViewFrom_overt_semitones440 =
		        (our instancePref_pitch_viewFrom () <
		                                our instancePref_pitch_viewTo ()
		                        ? our instancePref_pitch_viewFrom ()
		                        : pitchFloor_overt_semitones440);
		const double pitchViewTo_overt_semitones440 =
		        (our instancePref_pitch_viewFrom () <
		                                our instancePref_pitch_viewTo ()
		                        ? our instancePref_pitch_viewTo ()
		                        : pitchCeiling_overt_semitones440);
		const double pitchViewFrom_hidden_semitones440 =
		        Function_isUnitLogarithmic (Thing_dummyObject (Pitch),
		                Pitch_LEVEL_FREQUENCY, (int) kPitch_unit::SEMITONES_440)
		                ? log10 (pitchViewFrom_overt_semitones440)
		                : pitchViewFrom_overt_semitones440;
		const double pitchViewTo_hidden_semitones440 =
		        Function_isUnitLogarithmic (Thing_dummyObject (Pitch),
		                Pitch_LEVEL_FREQUENCY, (int) kPitch_unit::SEMITONES_440)
		                ? log10 (pitchViewTo_overt_semitones440)
		                : pitchViewTo_overt_semitones440;
		our d_pitchGrid_cursor =
		        localY_fraction * pitchViewTo_hidden_semitones440 +
		        (1.0 - localY_fraction) * pitchViewFrom_hidden_semitones440;
	...
	if (event->isDrop () && event->capsKeyPressed &&
	        our d_clickToChangePitchCallback && our d_changedBoss) {
		// Round to nearest resolution step (e.g., 0.05: 60.65, 72.20, 69.35)
		double pitchValue = our d_pitchGrid_cursor + 69.0;
		double resolution = our tierNotesFrequencyResolution;
		double roundedPitch = round (pitchValue / resolution) * resolution;
		our d_clickToChangePitchCallback (our d_changedBoss, roundedPitch);
	}

	return FunctionEditor_defaultMouseInWideDataView (
	        our functionEditor (), event, x_world);
}

bool SoundAnalysisArea_mouse (SoundAnalysisArea me,
        GuiDrawingArea_MouseEvent event, double x_world,
        double globalY_fraction) {
	const double localY_fraction =
	        my y_fraction_globalToLocal (globalY_fraction);
...
void structSoundAnalysisArea ::v1_info () {
	MelderInfo_writeLine (U"Longest analysis: ",
	        our instancePref_longestAnalysis (), U" seconds");
	if (v_hasSpectrogram ()) {
		/* Spectrogram flag: */
		MelderInfo_writeLine (
		        U"Spectrogram show: ", our instancePref_spectrogram_show ());
		/* Spectrogram settings: */
		MelderInfo_writeLine (U"Spectrogram view from: ",
		        our instancePref_spectrogram_viewFrom (), U" Hz");
		MelderInfo_writeLine (U"Spectrogram view to: ",
		        our instancePref_spectrogram_viewTo (), U" Hz");
		MelderInfo_writeLine (U"Spectrogram window length: ",
		        our instancePref_spectrogram_windowLength (), U" seconds");
		MelderInfo_writeLine (U"Spectrogram dynamic range: ",
		        our instancePref_spectrogram_dynamicRange (), U" dB");
		/* Advanced spectrogram settings: */
		MelderInfo_writeLine (U"Spectrogram number of time steps: ",
		        our instancePref_spectrogram_timeSteps ());
		MelderInfo_writeLine (U"Spectrogram number of frequency steps: ",
		        our instancePref_spectrogram_frequencySteps ());
		MelderInfo_writeLine (U"Spectrogram method: ", U"Fourier");
		MelderInfo_writeLine (U"Spectrogram window shape: ",
		        kSound_to_Spectrogram_windowShape_getText (
		                our instancePref_spectrogram_windowShape ()));
		MelderInfo_writeLine (U"Spectrogram autoscaling: ",
		        our instancePref_spectrogram_autoscaling ());
		MelderInfo_writeLine (U"Spectrogram maximum: ",
		        our instancePref_spectrogram_maximum (), U" dB/Hz");
		MelderInfo_writeLine (U"Spectrogram pre-emphasis: ",
		        our instancePref_spectrogram_preemphasis (), U" dB/octave");
		MelderInfo_writeLine (U"Spectrogram dynamicCompression: ",
		        our instancePref_spectrogram_dynamicCompression ());
		/* Dynamic information: */
		MelderInfo_writeLine (U"Spectrogram cursor frequency: ",
		        our d_spectrogram_cursor, U" Hz");
	}
	if (v_hasPitch ()) {
		/* Pitch flag: */
		...
		/* Pitch settings: */
		MelderInfo_writeLine (
		        U"Pitch floor: ", our instancePref_pitch_floor (), U" Hz");
		MelderInfo_writeLine (
		        U"Pitch ceiling: ", our instancePref_pitch_ceiling (), U" Hz");
		MelderInfo_writeLine (U"Pitch unit: ",
		        Function_getUnitText (Thing_dummyObject (Pitch),
		                Pitch_LEVEL_FREQUENCY, (int) instancePref_pitch_unit (),
		                Function_UNIT_TEXT_MENU));
		MelderInfo_writeLine (U"Pitch drawing method: ",
		        kSoundAnalysisArea_pitch_drawingMethod_getText (
		                instancePref_pitch_drawingMethod ()));
		/* Advanced pitch settings: */
		MelderInfo_writeLine (U"Pitch view from: ",
		        our instancePref_pitch_viewFrom (), U" ",
		        Function_getUnitText (Thing_dummyObject (Pitch),
		                Pitch_LEVEL_FREQUENCY,
		                (int) our instancePref_pitch_unit (),
		                Function_UNIT_TEXT_MENU));
		MelderInfo_writeLine (U"Pitch view to: ",
		        our instancePref_pitch_viewTo (), U" ",
		        Function_getUnitText (Thing_dummyObject (Pitch),
		                Pitch_LEVEL_FREQUENCY,
		                (int) our instancePref_pitch_unit (),
		                Function_UNIT_TEXT_MENU));
		MelderInfo_writeLine (U"Pitch method: ",
		        kSoundAnalysisArea_pitch_analysisMethod_getText (
		                our instancePref_pitch_method ()));
		MelderInfo_writeLine (U"Pitch very accurate: ",
		        our instancePref_pitch_veryAccurate ());
		MelderInfo_writeLine (U"Pitch max. number of candidates: ",
		        our instancePref_pitch_maximumNumberOfCandidates ());
		MelderInfo_writeLine (U"Pitch silence threshold: ",
		        our instancePref_pitch_silenceThreshold (), U" of global peak");
		MelderInfo_writeLine (U"Pitch voicing threshold: ",
		        our instancePref_pitch_voicingThreshold (),
		        U" (periodic power / total power)");
		MelderInfo_writeLine (U"Pitch octave cost: ",
		        our instancePref_pitch_octaveCost (), U" per octave");
		MelderInfo_writeLine (U"Pitch octave jump cost: ",
		        our instancePref_pitch_octaveJumpCost (), U" per octave");
		MelderInfo_writeLine (U"Pitch voiced/unvoiced cost: ",
		        our instancePref_pitch_voicedUnvoicedCost ());
	}
	if (v_hasIntensity ()) {
		/* Intensity flag: */
		MelderInfo_writeLine (
		        U"Intensity show: ", our instancePref_intensity_show ());
		/* Intensity settings: */
		MelderInfo_writeLine (U"Intensity view from: ",
		        our instancePref_intensity_viewFrom (), U" dB");
		MelderInfo_writeLine (U"Intensity view to: ",
		        our instancePref_intensity_viewTo (), U" dB");
		MelderInfo_writeLine (U"Intensity averaging method: ",
		        kSoundAnalysisArea_intensity_averagingMethod_getText (
		                our instancePref_intensity_averagingMethod ()));
		MelderInfo_writeLine (U"Intensity subtract mean pressure: ",
		        our instancePref_intensity_subtractMeanPressure ());
	}
	if (v_hasFormants ()) {
		/* Formant flag: */
		MelderInfo_writeLine (
		        U"Formant show: ", our instancePref_formant_show ());
		/* Formant settings: */
		...
		MelderInfo_writeLine (U"Formant number of poles: ",
		        Melder_iround (
		                2.0 *
		                our instancePref_formant_numberOfFormants ()));   // should
		// be
		// a
		// whole
		// number
		MelderInfo_writeLine (U"Formant window length: ",
		        our instancePref_formant_windowLength (), U" seconds");
		MelderInfo_writeLine (U"Formant dynamic range: ",
		        our instancePref_formant_dynamicRange (), U" dB");
		MelderInfo_writeLine (U"Formant dot size: ",
		        our instancePref_formant_dotSize (), U" mm");
		/* Advanced formant settings: */
		MelderInfo_writeLine (U"Formant method: ",
		        kSoundAnalysisArea_formant_analysisMethod_getText (
		                our instancePref_formant_method ()));
		MelderInfo_writeLine (U"Formant pre-emphasis from: ",
		        our instancePref_formant_preemphasisFrom (), U" Hz");
	}
	if (v_hasPulses ()) {
		/* Pulses flag: */
		MelderInfo_writeLine (
		        U"Pulses show: ", our instancePref_pulses_show ());
		MelderInfo_writeLine (U"Pulses maximum period factor: ",
		        our instancePref_pulses_maximumPeriodFactor ());
		MelderInfo_writeLine (U"Pulses maximum amplitude factor: ",
		        our instancePref_pulses_maximumAmplitudeFactor ());
	}
	const conststring32 logTargetTexts[3] = {
	        U"log file only", U"Info window only", U"log file and Info window"};
	const conststring32 log1TargetText =
	        logTargetTexts[our instancePref_log1_toLogFile () +
	                       2 * our instancePref_log1_toInfoWindow () - 1];
	...
	const conststring32 log2TargetText =
	        logTargetTexts[our instancePref_log2_toLogFile () +
	                       2 * our instancePref_log2_toInfoWindow () - 1];
...
static void menu_cb_showAnalyses (SoundAnalysisArea me, EDITOR_ARGS) {
	...
	BOOLEAN (showSpectrogram, U"Show spectrogram",
	        my default_spectrogram_show ())
	...
	POSITIVE (longestAnalysis, U"Longest analysis (s)",
	        my default_longestAnalysis ())
	EDITOR_OK
...
static void menu_cb_timeStepSettings (SoundAnalysisArea me, EDITOR_ARGS) {
	...
	POSITIVE (
	        fixedTimeStep, U"Fixed time step (s)", my default_fixedTimeStep ())
	LABEL (U"")
	LABEL (U"If the time step strategy is \"view-dependent\":")
	NATURAL (numberOfTimeStepsPerView, U"Number of time steps per view",
	        my default_numberOfTimeStepsPerView ())
	EDITOR_OK
	SET_ENUM (timeStepStrategy, kSoundAnalysisArea_timeStepStrategy,
	        my instancePref_timeStepStrategy ())
	SET_REAL (fixedTimeStep, my instancePref_fixedTimeStep ())
	SET_INTEGER (numberOfTimeStepsPerView,
	        my instancePref_numberOfTimeStepsPerView ())
	EDITOR_DO
...
static int makeQueriable (
        SoundAnalysisArea me, bool allowCursor, double *tmin, double *tmax) {
	if (my endWindow () - my startWindow () >
	        my instancePref_longestAnalysis ())
		Melder_throw (U"Window too long to show analyses. "
		              U"Zoom in to at most ",
		        Melder_half (my instancePref_longestAnalysis ()),
		        U" seconds "
		        U"or set the \"longest analysis\" to at least ",
		        Melder_half (my endWindow () - my startWindow ()),
	...
	} else if (my startSelection () < my startWindow () ||
	           my endSelection () > my endWindow ()) {
		Melder_throw (U"Command ambiguous: a part of the selection (",
		        my startSelection (), U", ", my endSelection (),
		        U") "
		        U"is outside of the window (",
		        my startWindow (), U", ", my endWindow (),
		        U"). "
...
static void menu_cb_logSettings (SoundAnalysisArea me, EDITOR_ARGS) {
	...
	EDITOR_OK
	SET_OPTION (writeLog1To, my instancePref_log1_toLogFile () +
	                                 2 * my instancePref_log1_toInfoWindow ())
	SET_STRING (logFile1, my instancePref_log1_fileName ())
	SET_STRING (log1format, my instancePref_log1_format ())
	SET_OPTION (writeLog2To, my instancePref_log2_toLogFile () +
	                                 2 * my instancePref_log2_toInfoWindow ())
...
static void do_log (SoundAnalysisArea me, int which) {
	...
	str32cpy (format, which == 1 ? my instancePref_log1_format ()
	                             : my instancePref_log2_format ());
	for (char32 *p = format; *p != U'\0'; p++)
		if (*p == U'\'') {
			/*
			...
			*s = U'\0';   // trailing null byte
			...
			if (str32equ (varName, U"time")) {
				...
				if (part == SoundAnalysisArea_PART_CURSOR)
					value = Pitch_getValueAtTime (my d_pitch.get (), tmin,
					        my instancePref_pitch_unit (), 1);
				else
					value = Pitch_getMean (my d_pitch.get (), tmin, tmax,
					        my instancePref_pitch_unit ());
			} else if (varName[0] == U'f' && varName[1] >= U'1' &&
			           varName[1] <= U'5' && varName[2] == U'\0') {
				SoundAnalysisArea_haveVisibleFormants (me);
				if (part == SoundAnalysisArea_PART_CURSOR)
					value = Formant_getValueAtTime (my d_formant.get (),
					        (int) (varName[1] - U'0'), tmin,
					        kFormant_unit::HERTZ);
				else
					value = Formant_getMean (my d_formant.get (),
					        (int) (varName[1] - U'0'), tmin, tmax,
					        kFormant_unit::HERTZ);
			} else if (varName[0] == U'b' && varName[1] >= U'1' &&
			           varName[1] <= U'5' && varName[2] == U'\0') {
				SoundAnalysisArea_haveVisibleFormants (me);
				value = Formant_getBandwidthAtTime (my d_formant.get (),
				        (int) (varName[1] - U'0'), 0.5 * (tmin + tmax),
				        kFormant_unit::HERTZ);
			} else if (str32equ (varName, U"intensity")) {
				SoundAnalysisArea_haveVisibleIntensity (me);
				if (part == SoundAnalysisArea_PART_CURSOR)
					value = Vector_getValueAtX (my d_intensity.get (), tmin,
					        Vector_CHANNEL_1,
					        kVector_valueInterpolation ::LINEAR);
				else
					value = Intensity_getAverage (my d_intensity.get (), tmin,
					        tmax,
					        (int) my instancePref_intensity_averagingMethod ());
			...
			if (isdefined (value)) {
				...
				if (precision >= 0)
					Melder_sprint (formattedNumber, 400,
					        Melder_fixed (value, precision));
				else
				...
				const integer varlen = (q - p) - 1, headlen = p - format,
				              arglen = Melder_length (stringValue);
				static MelderString buffer;
				MelderString_ncopy (&buffer, format, headlen);
	...
	if (which == 1 && my instancePref_log1_toInfoWindow () ||
	        which == 2 && my instancePref_log2_toInfoWindow ()) {
		MelderInfo_write (format);
		MelderInfo_close ();
	}
	if (which == 1 && my instancePref_log1_toLogFile () ||
	        which == 2 && my instancePref_log2_toLogFile ()) {
		structMelderFile file{};
		str32cat (format, U"\n");
		Melder_relativePathToFile (which == 1
		                                   ? my instancePref_log1_fileName ()
		                                   : my instancePref_log2_fileName (),
		        &file);
...
static void menu_cb_logScript3 (SoundAnalysisArea me, EDITOR_ARGS) {
	VOID_EDITOR
	praat_executeScriptFromEditorCommand (
	        my functionEditor (), cmd, my instancePref_logScript3 ());
	VOID_EDITOR_END
}
static void menu_cb_logScript4 (SoundAnalysisArea me, EDITOR_ARGS) {
	VOID_EDITOR
	praat_executeScriptFromEditorCommand (
	        my functionEditor (), cmd, my instancePref_logScript4 ());
	VOID_EDITOR_END
}
...
static void menu_cb_showSpectrogram (SoundAnalysisArea me, EDITOR_ARGS) {
	VOID_EDITOR
	my setInstancePref_spectrogram_show (
	        !my instancePref_spectrogram_show ());   // toggle
	GuiMenuItem_check (my spectrogramToggle,
	        my instancePref_spectrogram_show ());   // in case we're called from
	// a script
	FunctionEditor_redraw (my functionEditor ());
	VOID_EDITOR_END
}
...
static void menu_cb_spectrogramSettings (SoundAnalysisArea me, EDITOR_ARGS) {
	EDITOR_FORM (
	        U"Spectrogram settings", U"Intro 3.2. Configuring the spectrogram")
	REAL (viewFrom, U"left View range (Hz)", my default_spectrogram_viewFrom ())
	POSITIVE (
	        viewTo, U"right View range (Hz)", my default_spectrogram_viewTo ())
	POSITIVE (windowLength, U"Window length (s)",
	        my default_spectrogram_windowLength ())
	POSITIVE (dynamicRange, U"Dynamic range (dB)",
	        my default_spectrogram_dynamicRange ())
	...
	if (my instancePref_spectrogram_timeSteps () !=
	                Melder_atof (my default_spectrogram_timeSteps ()) ||
	        my instancePref_spectrogram_frequencySteps () !=
	                Melder_atof (my default_spectrogram_frequencySteps ()) ||
	        my instancePref_spectrogram_method () !=
	                my default_spectrogram_method () ||
	        my instancePref_spectrogram_windowShape () !=
	                my default_spectrogram_windowShape () ||
	        my instancePref_spectrogram_maximum () !=
	                Melder_atof (my default_spectrogram_maximum ()) ||
	        my instancePref_spectrogram_autoscaling () !=
	                my default_spectrogram_autoscaling () ||
	        my instancePref_spectrogram_preemphasis () !=
	                Melder_atof (my default_spectrogram_preemphasis ()) ||
	        my instancePref_spectrogram_dynamicCompression () !=
	                Melder_atof (
	                        my default_spectrogram_dynamicCompression ())) {
		SET_STRING (
		        note1, U"Warning: you have non-standard \"advanced settings\".")
	} else {
		SET_STRING (note1, U"(all of your \"advanced settings\" have their "
		                   U"standard values)")
	}
	if (my instancePref_timeStepStrategy () != my default_timeStepStrategy ()) {
		SET_STRING (note2,
		        U"Warning: you have a non-standard \"time step strategy\".")
	} else {
		SET_STRING (note2, U"(your \"time step strategy\" has its standard "
		                   U"value: automatic)")
	}
	EDITOR_DO
	Melder_require (viewFrom < viewTo,
	        U"The ceiling of the spectrogram view range "
	        U"should be greater than the floor.");
...
static void menu_cb_advancedSpectrogramSettings (
        SoundAnalysisArea me, EDITOR_ARGS) {
	EDITOR_FORM (U"Advanced spectrogram settings",
	        U"Advanced spectrogram settings...")
	LABEL (U"Time and frequency resolutions:")
	NATURAL (numberOfTimeSteps, U"Number of time steps",
	        my default_spectrogram_timeSteps ())
	NATURAL (numberOfFrequencySteps, U"Number of frequency steps",
	        my default_spectrogram_frequencySteps ())
	LABEL (U"Spectrogram analysis settings:")
	OPTIONMENU_ENUM (kSound_to_Spectrogram_method, method, U"Method",
	        my default_spectrogram_method ())
	...
	REAL (preemphasis, U"Pre-emphasis (dB/oct)",
	        my default_spectrogram_preemphasis ())
	REAL (dynamicCompression, U"Dynamic compression (0-1)",
	        my default_spectrogram_dynamicCompression ())
	EDITOR_OK
	SET_INTEGER (numberOfTimeSteps, my instancePref_spectrogram_timeSteps ())
	SET_INTEGER (numberOfFrequencySteps,
	        my instancePref_spectrogram_frequencySteps ())
	SET_ENUM (method, kSound_to_Spectrogram_method,
	        my instancePref_spectrogram_method ())
	SET_ENUM (windowShape, kSound_to_Spectrogram_windowShape,
	        my instancePref_spectrogram_windowShape ())
...
static void QUERY_EDITOR_FOR_REAL__getFrequency (
        SoundAnalysisArea me, EDITOR_ARGS) {
...
static void QUERY_DATA_FOR_REAL__getSpectralPowerAtCursorCross (
        SoundAnalysisArea me, EDITOR_ARGS) {
	QUERY_DATA_FOR_REAL
	double tmin, tmax;
	const int part = makeQueriable (me, true, &tmin, &tmax);
	...
	if (part != SoundAnalysisArea_PART_CURSOR)
		Melder_throw (U"Click inside the spectrogram first.");
	const double result = Matrix_getValueAtXY (
	        my d_spectrogram.get (), tmin, my d_spectrogram_cursor);
	QUERY_DATA_FOR_REAL_END (U" Pa2/Hz (at time = ", tmin,
	        U" seconds and frequency = ", my d_spectrogram_cursor, U" Hz)");
}
...
static void CONVERT_DATA_TO_ONE__ExtractVisibleSpectrogram (
        SoundAnalysisArea me, EDITOR_ARGS) {
	CONVERT_DATA_TO_ONE
	SoundAnalysisArea_haveVisibleSpectrogram (me);
...
static void CONVERT_DATA_TO_ONE__ViewSpectralSlice (
        SoundAnalysisArea me, EDITOR_ARGS) {
	CONVERT_DATA_TO_ONE
	const double start =
	...
	Sound_multiplyByWindow (sound.get (),
	        my instancePref_spectrogram_windowShape () ==
	                        kSound_to_Spectrogram_windowShape::SQUARE
	                ? kSound_windowShape::RECTANGULAR
	        : my instancePref_spectrogram_windowShape () ==
	                        kSound_to_Spectrogram_windowShape::HAMMING
	                ? kSound_windowShape::HAMMING
	        : my instancePref_spectrogram_windowShape () ==
	                        kSound_to_Spectrogram_windowShape::BARTLETT
	                ? kSound_windowShape::TRIANGULAR
	        : my instancePref_spectrogram_windowShape () ==
	                        kSound_to_Spectrogram_windowShape::WELCH
	                ? kSound_windowShape::PARABOLIC
	        : my instancePref_spectrogram_windowShape () ==
	                        kSound_to_Spectrogram_windowShape::HANNING
	...
	CONVERT_DATA_TO_ONE_END (Melder_cat (
	        (my data () ? my data ()->name.get () : U"untitled"), U"_",
	        Melder_fixed (
	                0.5 * (my startSelection () + my endSelection ()), 3)))
}

static void menu_cb_paintVisibleSpectrogram (
        SoundAnalysisArea me, EDITOR_ARGS) {
	...
	Spectrogram_paint (my d_spectrogram.get (), my pictureGraphics (),
...
#pragma mark - SoundAnalysisArea CQT menu

static void menu_cb_showCQT (SoundAnalysisArea me, EDITOR_ARGS) {
	VOID_EDITOR
...
static void menu_cb_cqtSettings (SoundAnalysisArea me, EDITOR_ARGS) {
...
static void menu_cb_showPitch (SoundAnalysisArea me, EDITOR_ARGS) {
	...
	GuiMenuItem_check (my pitchToggle,
	        my instancePref_pitch_show ());   // in case we're called from a
	// script
	FunctionEditor_redraw (my functionEditor ());
	VOID_EDITOR_END
}
...
static void menu_cb_showPitchGrid (SoundAnalysisArea me, EDITOR_ARGS) {
	VOID_EDITOR
	my setInstancePref_pitchGrid_show (
	        !my instancePref_pitchGrid_show ());   // toggle
	GuiMenuItem_check (my pitchGridToggle,
	        my instancePref_pitchGrid_show ());   // in case we're called from a
	// script
	FunctionEditor_redraw (my functionEditor ());
	VOID_EDITOR_END
}
...
static void menu_cb_pitchSettings (SoundAnalysisArea me, EDITOR_ARGS) {
	...
	POSITIVE (pitchCeiling, U"right Pitch range (Hz)",
	        my default_pitch_ceiling ())
	...
	OPTIONMENU_ENUM (kSoundAnalysisArea_pitchGrid_key, pitchGridKey,
	        U"Key signature", my default_pitchGrid_key ())
	BOOLEAN (pitchGridShowNoteNames, U"Show note names",
	        my default_pitchGrid_showNoteNames ())
	...
	SET_ENUM (analysisMethod, kSoundAnalysisArea_pitch_analysisMethod,
	        my instancePref_pitch_method ())
	...
	SET_BOOLEAN (
	        pitchGridShowNoteNames, my instancePref_pitchGrid_showNoteNames ())

	if (my instancePref_pitch_viewFrom () !=
	                Melder_atof (my default_pitch_viewFrom ()) ||
	        my instancePref_pitch_viewTo () !=
	                Melder_atof (my default_pitch_viewTo ()) ||
	        my instancePref_pitch_veryAccurate () !=
	                my default_pitch_veryAccurate () ||
	        my instancePref_pitch_maximumNumberOfCandidates () !=
	                Melder_atoi (
	                        my default_pitch_maximumNumberOfCandidates ()) ||
	        my instancePref_pitch_silenceThreshold () !=
	                Melder_atof (my default_pitch_silenceThreshold ()) ||
	        my instancePref_pitch_voicingThreshold () !=
	                Melder_atof (my default_pitch_voicingThreshold ()) ||
	        my instancePref_pitch_octaveCost () !=
	                Melder_atof (my default_pitch_octaveCost ()) ||
	        my instancePref_pitch_octaveJumpCost () !=
	                Melder_atof (my default_pitch_octaveJumpCost ()) ||
	        my instancePref_pitch_voicedUnvoicedCost () !=
	                Melder_atof (my default_pitch_voicedUnvoicedCost ())) {
		SET_STRING (note1,
		        U"Warning: you have some non-standard \"advanced settings\".")
	} else {
		SET_STRING (note1, U"(all of your \"advanced settings\" have their "
		                   U"standard values)")
	}
	...
	Melder_require (pitchCeiling > pitchFloor,
	        U"The pitch ceiling has to be greater than the pitch floor, so "
	        U"they cannot be ",
	        pitchCeiling, U" and ", pitchFloor, U" ",
	        kPitch_unit_getText (unit), U", respectively.");
...
static void menu_cb_advancedPitchSettings (SoundAnalysisArea me, EDITOR_ARGS) {
	...
	NATURAL (maximumNumberOfCandidates, U"Max. number of candidates",
	        my default_pitch_maximumNumberOfCandidates ())
	REAL (silenceThreshold, U"Silence threshold",
	        my default_pitch_silenceThreshold ())
	REAL (voicingThreshold, U"Voicing threshold",
	        my default_pitch_voicingThreshold ())
	REAL (octaveCost, U"Octave cost", my default_pitch_octaveCost ())
	REAL (octaveJumpCost, U"Octave-jump cost",
	        my default_pitch_octaveJumpCost ())
	REAL (voicedUnvoicedCost, U"Voiced / unvoiced cost",
	        my default_pitch_voicedUnvoicedCost ())
	EDITOR_OK
	...
	SET_INTEGER (maximumNumberOfCandidates,
	        my instancePref_pitch_maximumNumberOfCandidates ())
	...
	my setInstancePref_pitch_maximumNumberOfCandidates (
...
static void INFO_DATA__pitchListing (SoundAnalysisArea me, EDITOR_ARGS) {
	...
	MelderInfo_writeLine (U"Time_s   F0_",
	        Function_getUnitText (my d_pitch.get (), Pitch_LEVEL_FREQUENCY,
	                (int) my instancePref_pitch_unit (),
	                Function_UNIT_TEXT_SHORT));
	if (part == SoundAnalysisArea_PART_CURSOR) {
		double f0 = Pitch_getValueAtTime (
		        my d_pitch.get (), tmin, my instancePref_pitch_unit (), true);
		f0 = Function_convertToNonlogarithmic (my d_pitch.get (), f0,
		        Pitch_LEVEL_FREQUENCY, (int) my instancePref_pitch_unit ());
		MelderInfo_writeLine (
		        Melder_fixed (tmin, 6), U"   ", Melder_fixed (f0, 6));
	} else {
		integer i, i1, i2;
		...
		for (i = i1; i <= i2; i++) {
			...
			f0 = Function_convertToNonlogarithmic (my d_pitch.get (), f0,
			        Pitch_LEVEL_FREQUENCY, (int) my instancePref_pitch_unit ());
			MelderInfo_writeLine (
			        Melder_fixed (t, 6), U"   ", Melder_fixed (f0, 6));
		}
	}
	MelderInfo_close ();
	INFO_DATA_END
...
static void QUERY_DATA_FOR_REAL__getPitch (SoundAnalysisArea me, EDITOR_ARGS) {
	...
	const int part = makeQueriable (me, true, &tmin, &tmax);
	SoundAnalysisArea_haveVisiblePitch (me);
	double result = part == SoundAnalysisArea_PART_CURSOR
	                        ? Pitch_getValueAtTime (my d_pitch.get (), tmin,
	                                  my instancePref_pitch_unit (), true)
	                        : Pitch_getMean (my d_pitch.get (), tmin, tmax,
	                                  my instancePref_pitch_unit ());
	result = Function_convertToNonlogarithmic (my d_pitch.get (), result,
	        Pitch_LEVEL_FREQUENCY, (int) my instancePref_pitch_unit ());
	QUERY_DATA_FOR_REAL_END (U" ",
	        Function_getUnitText (my d_pitch.get (), Pitch_LEVEL_FREQUENCY,
	                (int) my instancePref_pitch_unit (), 0),
	        part == SoundAnalysisArea_PART_CURSOR
	                ? U" (interpolated pitch at CURSOR)"
	                : Melder_cat (U" (mean pitch ",
	                          SoundAnalysisArea_partString_locative (part),
	                          U")"));
}

static void QUERY_DATA_FOR_REAL__getMinimumPitch (
        SoundAnalysisArea me, EDITOR_ARGS) {
	QUERY_DATA_FOR_REAL
	double tmin, tmax;
	const int part = makeQueriable (me, false, &tmin, &tmax);
	SoundAnalysisArea_haveVisiblePitch (me);
	double result = Pitch_getMinimum (
	        my d_pitch.get (), tmin, tmax, my instancePref_pitch_unit (), true);
	result = Function_convertToNonlogarithmic (my d_pitch.get (), result,
	        Pitch_LEVEL_FREQUENCY, (int) my instancePref_pitch_unit ());
	QUERY_DATA_FOR_REAL_END (U" ",
	        Function_getUnitText (my d_pitch.get (), Pitch_LEVEL_FREQUENCY,
	                (int) my instancePref_pitch_unit (), 0),
	        U" (minimum pitch ", SoundAnalysisArea_partString_locative (part),
	        U")");
}

static void QUERY_DATA_FOR_REAL__getMaximumPitch (
        SoundAnalysisArea me, EDITOR_ARGS) {
	QUERY_DATA_FOR_REAL
	double tmin, tmax;
	const int part = makeQueriable (me, false, &tmin, &tmax);
	SoundAnalysisArea_haveVisiblePitch (me);   // BUG
	double result = Pitch_getMaximum (
	        my d_pitch.get (), tmin, tmax, my instancePref_pitch_unit (), true);
	result = Function_convertToNonlogarithmic (my d_pitch.get (), result,
	        Pitch_LEVEL_FREQUENCY, (int) my instancePref_pitch_unit ());
	QUERY_DATA_FOR_REAL_END (U" ",
	        Function_getUnitText (my d_pitch.get (), Pitch_LEVEL_FREQUENCY,
	                (int) my instancePref_pitch_unit (), 0),
	        U" (maximum pitch ", SoundAnalysisArea_partString_locative (part),
	        U")");
}

static void menu_cb_moveCursorToMinimumPitch (
        SoundAnalysisArea me, EDITOR_ARGS) {
	VOID_EDITOR
	SoundAnalysisArea_haveVisiblePitch (me);
	if (my startSelection () == my endSelection ()) {
		...
		double time;
		Pitch_getMinimumAndTime (my d_pitch.get (), my startSelection (),
		        my endSelection (), my instancePref_pitch_unit (), 1, nullptr,
		        &time);
		if (isundef (time)) Melder_throw (U"Selection is voiceless.");
		my setSelection (time, time);
		Melder_assert (isdefined (my startSelection ()));   // precondition of
...
static void menu_cb_moveCursorToMaximumPitch (
        SoundAnalysisArea me, EDITOR_ARGS) {
	VOID_EDITOR
	SoundAnalysisArea_haveVisiblePitch (me);
	if (my startSelection () == my endSelection ()) {
		...
		double time;
		Pitch_getMaximumAndTime (my d_pitch.get (), my startSelection (),
		        my endSelection (), my instancePref_pitch_unit (), 1, nullptr,
		        &time);
		if (isundef (time)) Melder_throw (U"Selection is voiceless.");
		my setSelection (time, time);
...
static void CONVERT_DATA_TO_ONE__ExtractVisiblePitchContour (
        SoundAnalysisArea me, EDITOR_ARGS) {
	CONVERT_DATA_TO_ONE
	SoundAnalysisArea_haveVisiblePitch (me);
...
static void menu_cb_drawVisiblePitchContour (
        SoundAnalysisArea me, EDITOR_ARGS) {
	...
	SoundAnalysisArea_haveVisiblePitch (me);
	DataGui_openPraatPicture (me);
	const double pitchFloor_hidden = Function_convertStandardToSpecialUnit (
	        my d_pitch.get (), my instancePref_pitch_floor (),
	        Pitch_LEVEL_FREQUENCY, (int) my instancePref_pitch_unit ());
	const double pitchCeiling_hidden = Function_convertStandardToSpecialUnit (
	        my d_pitch.get (), my instancePref_pitch_ceiling (),
	        Pitch_LEVEL_FREQUENCY, (int) my instancePref_pitch_unit ());
	const double pitchFloor_overt = Function_convertToNonlogarithmic (
	        my d_pitch.get (), pitchFloor_hidden, Pitch_LEVEL_FREQUENCY,
	        (int) my instancePref_pitch_unit ());
	const double pitchCeiling_overt = Function_convertToNonlogarithmic (
	        my d_pitch.get (), pitchCeiling_hidden, Pitch_LEVEL_FREQUENCY,
	        (int) my instancePref_pitch_unit ());
	const double pitchViewFrom_overt =
	        (my instancePref_pitch_viewFrom () < my instancePref_pitch_viewTo ()
	                        ? my instancePref_pitch_viewFrom ()
	                        : pitchFloor_overt);
	const double pitchViewTo_overt =
	        (my instancePref_pitch_viewFrom () < my instancePref_pitch_viewTo ()
	                        ? my instancePref_pitch_viewTo ()
	                        : pitchCeiling_overt);
	Pitch_draw (my d_pitch.get (), my pictureGraphics (), my startWindow (),
	        my endWindow (), pitchViewFrom_overt, pitchViewTo_overt, garnish,
...
static void menu_cb_showIntensity (SoundAnalysisArea me, EDITOR_ARGS) {
	VOID_EDITOR
	my setInstancePref_intensity_show (
	        !my instancePref_intensity_show ());   // toggle
	GuiMenuItem_check (my intensityToggle,
	        my instancePref_intensity_show ());   // in case we're called from a
	// script
	FunctionEditor_redraw (my functionEditor ());
	VOID_EDITOR_END
}
...
static void menu_cb_intensitySettings (SoundAnalysisArea me, EDITOR_ARGS) {
	EDITOR_FORM (U"Intensity settings",
	        U"Intro 6.2. Configuring the intensity contour")
	...
	BOOLEAN (subtractMeanPressure, U"Subtract mean pressure",
	        my default_intensity_subtractMeanPressure ())
	...
	SET_ENUM (averagingMethod, kSoundAnalysisArea_intensity_averagingMethod,
	        my instancePref_intensity_averagingMethod ())
	SET_BOOLEAN (subtractMeanPressure,
	        my instancePref_intensity_subtractMeanPressure ())
	if (my instancePref_timeStepStrategy () != my default_timeStepStrategy ()) {
		SET_STRING (note2,
		        U"Warning: you have a non-standard \"time step strategy\".")
...
static void CONVERT_DATA_TO_ONE__ExtractVisibleIntensityContour (
        SoundAnalysisArea me, EDITOR_ARGS) {
	CONVERT_DATA_TO_ONE
	SoundAnalysisArea_haveVisibleIntensity (me);
...
static void menu_cb_drawVisibleIntensityContour (
        SoundAnalysisArea me, EDITOR_ARGS) {
	...
	SoundAnalysisArea_haveVisibleIntensity (me);
	DataGui_openPraatPicture (me);
	Intensity_draw (my d_intensity.get (), my pictureGraphics (),
	        my startWindow (), my endWindow (),
	        my instancePref_intensity_viewFrom (),
	        my instancePref_intensity_viewTo (), garnish);
	FunctionArea_garnishPicture (me);
	DataGui_closePraatPicture (me);
	EDITOR_END
}
...
static void INFO_DATA__intensityListing (SoundAnalysisArea me, EDITOR_ARGS) {
	...
	if (part == SoundAnalysisArea_PART_CURSOR) {
		const double intensity = Vector_getValueAtX (my d_intensity.get (),
		        tmin, Vector_CHANNEL_1, kVector_valueInterpolation ::LINEAR);
		MelderInfo_writeLine (
		        Melder_fixed (tmin, 6), U"   ", Melder_fixed (intensity, 6));
	} else {
		integer i, i1, i2;
		...
		for (i = i1; i <= i2; i++) {
			...
			const double intensity = Vector_getValueAtX (my d_intensity.get (),
			        time, Vector_CHANNEL_1,
			        kVector_valueInterpolation ::NEAREST);
			MelderInfo_writeLine (Melder_fixed (time, 6), U"   ",
			        Melder_fixed (intensity, 6));
		}
	}
	MelderInfo_close ();
	INFO_DATA_END
...
static void QUERY_DATA_FOR_REAL__getIntensity (
        SoundAnalysisArea me, EDITOR_ARGS) {
	QUERY_DATA_FOR_REAL
	double tmin, tmax;
	const int part = makeQueriable (me, true, &tmin, &tmax);
	...
	const double result =
	        (part == SoundAnalysisArea_PART_CURSOR
	                        ? Vector_getValueAtX (my d_intensity.get (), tmin,
	                                  Vector_CHANNEL_1,
	                                  kVector_valueInterpolation ::LINEAR)
	                        : Intensity_getAverage (my d_intensity.get (), tmin,
	                                  tmax,
	                                  (int) my
	                                          instancePref_intensity_averagingMethod ()));
	static const conststring32 methodString[] = {
	        U"median", U"mean-energy", U"mean-sones", U"mean-dB"};
	QUERY_DATA_FOR_REAL_END (
	        part == SoundAnalysisArea_PART_CURSOR
...
static void QUERY_DATA_FOR_REAL__getMinimumIntensity (
        SoundAnalysisArea me, EDITOR_ARGS) {
	QUERY_DATA_FOR_REAL
	double tmin, tmax;
	const int part = makeQueriable (me, false, &tmin, &tmax);
	SoundAnalysisArea_haveVisibleIntensity (me);
	const double result = Vector_getMinimum (my d_intensity.get (), tmin, tmax,
	        kVector_peakInterpolation ::PARABOLIC);
	QUERY_DATA_FOR_REAL_END (U" dB (minimum intensity ",
	        SoundAnalysisArea_partString_locative (part), U")");
}

static void QUERY_DATA_FOR_REAL__getMaximumIntensity (
        SoundAnalysisArea me, EDITOR_ARGS) {
	QUERY_DATA_FOR_REAL
	double tmin, tmax;
	const int part = makeQueriable (me, false, &tmin, &tmax);
	SoundAnalysisArea_haveVisibleIntensity (me);
	const double result = Vector_getMaximum (my d_intensity.get (), tmin, tmax,
	        kVector_peakInterpolation ::PARABOLIC);
	QUERY_DATA_FOR_REAL_END (U" dB (maximum intensity ",
	        SoundAnalysisArea_partString_locative (part), U")");
}
...
static void menu_cb_showFormants (SoundAnalysisArea me, EDITOR_ARGS) {
	VOID_EDITOR
	my setInstancePref_formant_show (
	        !my instancePref_formant_show ());   // toggle
	GuiMenuItem_check (my formantToggle,
	        my instancePref_formant_show ());   // in case we're called from a
	// script
	FunctionEditor_redraw (my functionEditor ());
	VOID_EDITOR_END
}
...
static void menu_cb_formantSettings (SoundAnalysisArea me, EDITOR_ARGS) {
	EDITOR_FORM (
	        U"Formant settings", U"Intro 5.2. Configuring the formant contours")
	POSITIVE (formantCeiling, U"Formant ceiling (Hz)",
	        my default_formant_ceiling ())
	POSITIVE (numberOfFormants, U"Number of formants",
	        my default_formant_numberOfFormants ())
	POSITIVE (windowLength, U"Window length (s)",
	        my default_formant_windowLength ())
	REAL (dynamicRange, U"Dynamic range (dB)",
	        my default_formant_dynamicRange ())
	...
	if (my instancePref_formant_method () != my default_formant_method () ||
	        my instancePref_formant_preemphasisFrom () !=
	                Melder_atof (my default_formant_preemphasisFrom ())) {
		SET_STRING (
		        note1, U"Warning: you have non-standard \"advanced settings\".")
	} else {
		SET_STRING (note1, U"(all of your \"advanced settings\" have their "
...
static void menu_cb_advancedFormantSettings (
        SoundAnalysisArea me, EDITOR_ARGS) {
	EDITOR_FORM (U"Advanced formant settings", U"Advanced formant settings...")
	RADIO_ENUM (kSoundAnalysisArea_formant_analysisMethod, method, U"Method",
	        my default_formant_method ())
	POSITIVE (preemphasisFrom, U"Pre-emphasis from (Hz)",
	        my default_formant_preemphasisFrom ())
	EDITOR_OK
	SET_ENUM (method, kSoundAnalysisArea_formant_analysisMethod,
	        my instancePref_formant_method ())
...
static void CONVERT_DATA_TO_ONE__ExtractVisibleFormantContour (
        SoundAnalysisArea me, EDITOR_ARGS) {
	CONVERT_DATA_TO_ONE
	SoundAnalysisArea_haveVisibleFormants (me);
...
static void menu_cb_drawVisibleFormantContour (
        SoundAnalysisArea me, EDITOR_ARGS) {
	...
	SoundAnalysisArea_haveVisibleFormants (me);
	DataGui_openPraatPicture (me);
	Formant_drawSpeckles (my d_formant.get (), my pictureGraphics (),
	        my startWindow (), my endWindow (),
	        my instancePref_spectrogram_viewTo (),
	        my instancePref_formant_dynamicRange (), garnish);
	FunctionArea_garnishPicture (me);
	DataGui_closePraatPicture (me);
	EDITOR_END
}
...
static void INFO_DATA__formantListing (SoundAnalysisArea me, EDITOR_ARGS) {
	...
	if (part == SoundAnalysisArea_PART_CURSOR) {
		const double f1 = Formant_getValueAtTime (
		        my d_formant.get (), 1, tmin, kFormant_unit::HERTZ);
		const double f2 = Formant_getValueAtTime (
		        my d_formant.get (), 2, tmin, kFormant_unit::HERTZ);
		const double f3 = Formant_getValueAtTime (
		        my d_formant.get (), 3, tmin, kFormant_unit::HERTZ);
		const double f4 = Formant_getValueAtTime (
		        my d_formant.get (), 4, tmin, kFormant_unit::HERTZ);
		...
		for (integer i = i1; i <= i2; i++) {
			...
			const double f1 = Formant_getValueAtTime (
			        my d_formant.get (), 1, t, kFormant_unit::HERTZ);
			const double f2 = Formant_getValueAtTime (
			        my d_formant.get (), 2, t, kFormant_unit::HERTZ);
			const double f3 = Formant_getValueAtTime (
			        my d_formant.get (), 3, t, kFormant_unit::HERTZ);
			const double f4 = Formant_getValueAtTime (
			        my d_formant.get (), 4, t, kFormant_unit::HERTZ);
			MelderInfo_writeLine (Melder_fixed (t, 6), U"   ",
			        Melder_fixed (f1, 6), U"   ", Melder_fixed (f2, 6), U"   ",
			        Melder_fixed (f3, 6), U"   ", Melder_fixed (f4, 6));
		}
	}
...
static void do_getFormant (SoundAnalysisArea me, integer iformant,
	...
	const double result =
	        (part == SoundAnalysisArea_PART_CURSOR
	                        ? Formant_getValueAtTime (my d_formant.get (),
	                                  iformant, tmin, kFormant_unit::HERTZ)
	                        : Formant_getMean (my d_formant.get (), iformant,
	                                  tmin, tmax, kFormant_unit::HERTZ));
	QUERY_DATA_FOR_REAL_END (
	        part == SoundAnalysisArea_PART_CURSOR
	                ? Melder_cat (U" Hz (nearest F", iformant, U" to CURSOR)")
	                : Melder_cat (U" Hz (mean F", iformant, U" ",
	                          SoundAnalysisArea_partString_locative (part),
	                          U")"))
}
static void do_getBandwidth (SoundAnalysisArea me, integer iformant,
        Interpreter optionalInterpreter) {
	QUERY_DATA_FOR_REAL
	double tmin, tmax;
	const int part = makeQueriable (me, true, &tmin, &tmax);
	...
	const double result =
	        (part == SoundAnalysisArea_PART_CURSOR
	                        ? Formant_getBandwidthAtTime (my d_formant.get (),
	                                  iformant, tmin, kFormant_unit::HERTZ)
	                        : Formant_getBandwidthAtTime (my d_formant.get (),
	                                  iformant, 0.5 * (tmin + tmax),
	                                  kFormant_unit::HERTZ));
	QUERY_DATA_FOR_REAL_END (
	        part == SoundAnalysisArea_PART_CURSOR
	                ? Melder_cat (U" Hz (nearest B", iformant, U" to CURSOR)")
	                : Melder_cat (U" Hz (B", iformant, U" in centre of ",
	                          SoundAnalysisArea_partString (part), U")"))
}
static void QUERY_DATA_FOR_REAL__getFirstFormant (
        SoundAnalysisArea me, EDITOR_ARGS) {
	do_getFormant (me, 1, optionalInterpreter);
}
static void QUERY_DATA_FOR_REAL__getFirstBandwidth (
        SoundAnalysisArea me, EDITOR_ARGS) {
	do_getBandwidth (me, 1, optionalInterpreter);
}
static void QUERY_DATA_FOR_REAL__getSecondFormant (
        SoundAnalysisArea me, EDITOR_ARGS) {
	do_getFormant (me, 2, optionalInterpreter);
}
static void QUERY_DATA_FOR_REAL__getSecondBandwidth (
        SoundAnalysisArea me, EDITOR_ARGS) {
	do_getBandwidth (me, 2, optionalInterpreter);
}
static void QUERY_DATA_FOR_REAL__getThirdFormant (
        SoundAnalysisArea me, EDITOR_ARGS) {
	do_getFormant (me, 3, optionalInterpreter);
}
static void QUERY_DATA_FOR_REAL__getThirdBandwidth (
        SoundAnalysisArea me, EDITOR_ARGS) {
...
static void QUERY_DATA_FOR_REAL__getBandwidth (
        SoundAnalysisArea me, EDITOR_ARGS) {
...
static void menu_cb_showPulses (SoundAnalysisArea me, EDITOR_ARGS) {
	VOID_EDITOR
	my setInstancePref_pulses_show (
	        !my instancePref_pulses_show ());   // toggle
	GuiMenuItem_check (my pulsesToggle,
	        my instancePref_pulses_show ());   // in case we're called from a
	// script
	FunctionEditor_redraw (my functionEditor ());
	VOID_EDITOR_END
}
...
static void menu_cb_advancedPulsesSettings (SoundAnalysisArea me, EDITOR_ARGS) {
	...
	POSITIVE (maximumPeriodFactor, U"Maximum period factor",
	        my default_pulses_maximumPeriodFactor ())
	POSITIVE (maximumAmplitudeFactor, U"Maximum amplitude factor",
	        my default_pulses_maximumAmplitudeFactor ())
	EDITOR_OK
	SET_REAL (
	        maximumPeriodFactor, my instancePref_pulses_maximumPeriodFactor ())
	SET_REAL (maximumAmplitudeFactor,
	        my instancePref_pulses_maximumAmplitudeFactor ())
	EDITOR_DO
...
static void CONVERT_DATA_TO_ONE__ExtractVisiblePulses (
        SoundAnalysisArea me, EDITOR_ARGS) {
	CONVERT_DATA_TO_ONE
...
static void menu_cb_drawVisiblePulses (SoundAnalysisArea me, EDITOR_ARGS) {
	...
	SoundAnalysisArea_haveVisiblePulses (me);
	DataGui_openPraatPicture (me);
	PointProcess_draw (my d_pulses.get (), my pictureGraphics (),
	        my startWindow (), my endWindow (), garnish);
	FunctionArea_garnishPicture (me);
	DataGui_closePraatPicture (me);
	EDITOR_END
}
...
static void INFO_DATA__voiceReport (SoundAnalysisArea me, EDITOR_ARGS) {
	...
	MelderInfo_open ();
	MelderInfo_writeLine (U"-- Voice report for ", my name.get (),
	        U" --\nDate: ", Melder_peek8to32 (ctime (&today)));
	if (my instancePref_pitch_method () !=
	        kSoundAnalysisArea_pitch_analysisMethod::CROSS_CORRELATION)
		MelderInfo_writeLine (
		        U"WARNING: some of the following measurements may be "
		        U"imprecise.\n"
		        "For more precision, go to \"Pitch settings\" and choose\n"
		        "the cross-correlation analysis method to optimize for voice "
		        "research.\n");
	MelderInfo_writeLine (
	        U"Time range of ", SoundAnalysisArea_partString (part));
	Sound_Pitch_PointProcess_voiceReport (sound.get (), my d_pitch.get (),
	        my d_pulses.get (), tmin, tmax, my instancePref_pitch_floor (),
	        my instancePref_pitch_ceiling (),
	        my instancePref_pulses_maximumPeriodFactor (),
	        my instancePref_pulses_maximumAmplitudeFactor (),
	        my instancePref_pitch_silenceThreshold (),
	        my instancePref_pitch_voicingThreshold ());
	MelderInfo_close ();
...
/*
 static void cb_getJitter_xx (SoundAnalysisArea me, double
 (*PointProcess_getJitter_xx) (PointProcess, double, double, double, double,
 double)) { const double minimumPeriod = 0.8 / my p_pitch_ceiling, maximumPeriod
 ...
 cb_getJitter_local_absolute) { cb_getJitter_xx (me,
 PointProcess_getJitter_local_absolute); END } DIRECT (SoundAnalysisArea,
 cb_getJitter_rap) { cb_getJitter_xx (me, PointProcess_getJitter_rap); END }
 DIRECT (SoundAnalysisArea, cb_getJitter_ppq5) { cb_getJitter_xx (me,
 PointProcess_getJitter_ppq5); END } DIRECT (SoundAnalysisArea,
 cb_getJitter_ddp) { cb_getJitter_xx (me, PointProcess_getJitter_ddp); END }

 static void cb_getShimmer_xx (SoundAnalysisArea me, double
 (*PointProcess_Sound_getShimmer_xx) (PointProcess, Sound, double, double,
 double, double, double)) { const double minimumPeriod = 0.8 / my
 p_pitch_ceiling, maximumPeriod = 1.25 / my p_pitch_floor;
 SoundAnalysisArea_haveVisiblePulses (me);
 if (my startSelection() == my endSelection())
 Melder_throw (U"Make a selection first.");
 makeQueriable
 autoSound sound = extractSound (me, my startSelection(), my endSelection());
 Melder_informationReal (PointProcess_Sound_getShimmer_xx (my d_pulses,
 sound, my startSelection(), my endSelection(), minimumPeriod, maximumPeriod, my
 p_pulses_maximumAmplitudeFactor), nullptr);
 }
 DIRECT (SoundAnalysisArea, cb_getShimmer_local) { cb_getShimmer_xx (me,
 PointProcess_Sound_getShimmer_local); END } DIRECT (SoundAnalysisArea,
 cb_getShimmer_local_dB) { cb_getShimmer_xx (me,
 PointProcess_Sound_getShimmer_local_dB); END } DIRECT (SoundAnalysisArea,
 cb_getShimmer_apq3) { cb_getShimmer_xx (me,
 PointProcess_Sound_getShimmer_apq3); END } DIRECT (SoundAnalysisArea,
 cb_getShimmer_apq5) { cb_getShimmer_xx (me,
 PointProcess_Sound_getShimmer_apq5); END } DIRECT (SoundAnalysisArea,
 cb_getShimmer_apq11) { cb_getShimmer_xx (me,
 PointProcess_Sound_getShimmer_apq11); END } DIRECT (SoundAnalysisArea,
 cb_getShimmer_dda) { cb_getShimmer_xx (me, PointProcess_Sound_getShimmer_dda);
 END }
 */
...
void structSoundAnalysisArea ::v_createMenuItems_formant (EditorMenu menu) {
	our formantToggle = FunctionAreaMenu_addCommand (menu, U"Show formants",
	        GuiMenu_CHECKBUTTON |
	                (instancePref_formant_show () ? GuiMenu_TOGGLE_ON : 0),
	        menu_cb_showFormants, this);
	FunctionAreaMenu_addCommand (
	        menu, U"Formant settings...", 0, menu_cb_formantSettings, this);
	...
	FunctionAreaMenu_addCommand (
	        menu, U"Formant listing", 1, INFO_DATA__formantListing, this);
	FunctionAreaMenu_addCommand (menu, U"Get first formant",
	        GuiMenu_F1 | GuiMenu_DEPTH_1, QUERY_DATA_FOR_REAL__getFirstFormant,
	        this);
	FunctionAreaMenu_addCommand (menu, U"Get first bandwidth", 1,
	        QUERY_DATA_FOR_REAL__getFirstBandwidth, this);
	FunctionAreaMenu_addCommand (menu, U"Get second formant",
	        GuiMenu_F2 | GuiMenu_DEPTH_1, QUERY_DATA_FOR_REAL__getSecondFormant,
	        this);
	FunctionAreaMenu_addCommand (menu, U"Get second bandwidth", 1,
	        QUERY_DATA_FOR_REAL__getSecondBandwidth, this);
	FunctionAreaMenu_addCommand (menu, U"Get third formant",
	        GuiMenu_F3 | GuiMenu_DEPTH_1, QUERY_DATA_FOR_REAL__getThirdFormant,
	        this);
	FunctionAreaMenu_addCommand (menu, U"Get third bandwidth", 1,
	        QUERY_DATA_FOR_REAL__getThirdBandwidth, this);
	FunctionAreaMenu_addCommand (menu, U"Get fourth formant",
	        GuiMenu_F4 | GuiMenu_DEPTH_1, QUERY_DATA_FOR_REAL__getFourthFormant,
	        this);
	FunctionAreaMenu_addCommand (menu, U"Get fourth bandwidth", 1,
	        QUERY_DATA_FOR_REAL__getFourthBandwidth, this);
	FunctionAreaMenu_addCommand (
	        menu, U"Get formant...", 1, QUERY_DATA_FOR_REAL__getFormant, this);
	FunctionAreaMenu_addCommand (menu, U"Get bandwidth...", 1,
	        QUERY_DATA_FOR_REAL__getBandwidth, this);

	FunctionAreaMenu_addCommand (
	        menu, U"- Draw formants to picture window:", 0, nullptr, this);
	FunctionAreaMenu_addCommand (menu, U"Draw visible formant contour...", 1,
	        menu_cb_drawVisibleFormantContour, this);

	FunctionAreaMenu_addCommand (
	        menu, U"- Extract formants to objects window:", 0, nullptr, this);
...
void structSoundAnalysisArea ::v_createMenus () {
	...
	FunctionAreaMenu_addCommand (
	        menu, U"Show analyses...", 0, menu_cb_showAnalyses, this);
	FunctionAreaMenu_addCommand (
	        menu, U"Time step settings...", 0, menu_cb_timeStepSettings, this);
	FunctionAreaMenu_addCommand (menu, U"-- query log --", 0, nullptr, this);
	FunctionAreaMenu_addCommand (
	        menu, U"Log settings...", 0, menu_cb_logSettings, this);
	FunctionAreaMenu_addCommand (
	        menu, U"Delete log file 1", 0, menu_cb_deleteLogFile1, this);
	FunctionAreaMenu_addCommand (
	        menu, U"Delete log file 2", 0, menu_cb_deleteLogFile2, this);
	FunctionAreaMenu_addCommand (
	        menu, U"Log 1", GuiMenu_F12, menu_cb_log1, this);
	FunctionAreaMenu_addCommand (
	        menu, U"Log 2", GuiMenu_F12 | GuiMenu_SHIFT, menu_cb_log2, this);
	FunctionAreaMenu_addCommand (menu, U"Log script 3 (...)",
	        GuiMenu_F12 | GuiMenu_OPTION, menu_cb_logScript3, this);
	FunctionAreaMenu_addCommand (menu, U"Log script 4 (...)",
	        GuiMenu_F12 | GuiMenu_COMMAND, menu_cb_logScript4, this);

	if (our v_hasSpectrogram ()) {
		menu = Editor_addMenu (our functionEditor (), U"Spectrogram", 0);
		our spectrogramToggle = FunctionAreaMenu_addCommand (menu,
		        U"Show spectrogram",
		        GuiMenu_CHECKBUTTON |
		                (instancePref_spectrogram_show () ? GuiMenu_TOGGLE_ON
		                                                  : 0),
		        menu_cb_showSpectrogram, this);
		FunctionAreaMenu_addCommand (menu, U"Spectrogram settings...", 0,
		        menu_cb_spectrogramSettings, this);
		FunctionAreaMenu_addCommand (menu, U"Advanced spectrogram settings...",
		        0, menu_cb_advancedSpectrogramSettings, this);
		FunctionAreaMenu_addCommand (
		        menu, U"- Query spectrogram:", 0, nullptr, this);
		FunctionAreaMenu_addCommand (menu, U"Get frequency at frequency cursor",
		        1, QUERY_EDITOR_FOR_REAL__getFrequency, this);
		FunctionAreaMenu_addCommand (menu,
		        U"Get spectral power at cursor cross",
		        GuiMenu_F7 | GuiMenu_DEPTH_1,
		        QUERY_DATA_FOR_REAL__getSpectralPowerAtCursorCross, this);
		FunctionAreaMenu_addCommand (
		        menu, U"- Select frequency:", 0, nullptr, this);
		FunctionAreaMenu_addCommand (menu, U"Move frequency cursor to...", 1,
		        menu_cb_moveFrequencyCursorTo, this);
		FunctionAreaMenu_addCommand (menu,
		        U"- Draw spectogram to picture window:", 0, nullptr, this);
		FunctionAreaMenu_addCommand (menu, U"Paint visible spectrogram...", 1,
		        menu_cb_paintVisibleSpectrogram, this);
		FunctionAreaMenu_addCommand (
		        menu, U"- Extract to objects window:", 0, nullptr, this);
		FunctionAreaMenu_addCommand (menu, U"Extract visible spectrogram", 1,
		        CONVERT_DATA_TO_ONE__ExtractVisibleSpectrogram, this);
		FunctionAreaMenu_addCommand (menu, U"View spectral slice",
		        'L' | GuiMenu_DEPTH_1, CONVERT_DATA_TO_ONE__ViewSpectralSlice,
		        this);
	}

	// CQT menu
	menu = Editor_addMenu (our functionEditor (), U"CQT", 0);
	our cqtToggle = FunctionAreaMenu_addCommand (menu, U"Show CQT",
	        GuiMenu_CHECKBUTTON |
	                (instancePref_cqt_show () ? GuiMenu_TOGGLE_ON : 0),
	        menu_cb_showCQT, this);
	FunctionAreaMenu_addCommand (
	        menu, U"CQT settings...", 0, menu_cb_cqtSettings, this);

	if (our v_hasPitch ()) {
		menu = Editor_addMenu (our functionEditor (), U"Pitch", 0);
		our pitchToggle = FunctionAreaMenu_addCommand (menu, U"Show pitch",
		        GuiMenu_CHECKBUTTON |
		                (instancePref_pitch_show () ? GuiMenu_TOGGLE_ON : 0),
		        menu_cb_showPitch, this);
		our pitchGridToggle = FunctionAreaMenu_addCommand (menu,
		        U"Show pitch grid",
		        GuiMenu_CHECKBUTTON |
		                (instancePref_pitchGrid_show () ? GuiMenu_TOGGLE_ON
		                                                : 0),
		        menu_cb_showPitchGrid, this);
		FunctionAreaMenu_addCommand (
		        menu, U"Pitch settings...", 0, menu_cb_pitchSettings, this);
		...
		FunctionAreaMenu_addCommand (
		        menu, U"Pitch listing", 1, INFO_DATA__pitchListing, this);
		FunctionAreaMenu_addCommand (menu, U"Get pitch",
		        GuiMenu_F5 | GuiMenu_DEPTH_1, QUERY_DATA_FOR_REAL__getPitch,
		        this);
		FunctionAreaMenu_addCommand (menu, U"Get minimum pitch",
		        GuiMenu_F5 | GuiMenu_COMMAND | GuiMenu_DEPTH_1,
		        QUERY_DATA_FOR_REAL__getMinimumPitch, this);
		FunctionAreaMenu_addCommand (menu, U"Get maximum pitch",
		        GuiMenu_F5 | GuiMenu_SHIFT | GuiMenu_DEPTH_1,
		        QUERY_DATA_FOR_REAL__getMaximumPitch, this);
		FunctionAreaMenu_addCommand (
		        menu, U"- Select by pitch:", 0, nullptr, this);
		FunctionAreaMenu_addCommand (menu, U"Move cursor to minimum pitch",
		        GuiMenu_SHIFT | 'L' | GuiMenu_DEPTH_1,
		        menu_cb_moveCursorToMinimumPitch, this);
		FunctionAreaMenu_addCommand (menu, U"Move cursor to maximum pitch",
		        GuiMenu_SHIFT | 'H' | GuiMenu_DEPTH_1,
		        menu_cb_moveCursorToMaximumPitch, this);
		FunctionAreaMenu_addCommand (
		        menu, U"- Draw pitch to picture window:", 0, nullptr, this);
		FunctionAreaMenu_addCommand (menu, U"Draw visible pitch contour...", 1,
		        menu_cb_drawVisiblePitchContour, this);
		FunctionAreaMenu_addCommand (
		        menu, U"- Extract pitch to objects window:", 0, nullptr, this);
	...
	if (our v_hasIntensity ()) {
		...
		our intensityToggle = FunctionAreaMenu_addCommand (menu,
		        U"Show intensity",
		        GuiMenu_CHECKBUTTON |
		                (instancePref_intensity_show () ? GuiMenu_TOGGLE_ON
		                                                : 0),
		        menu_cb_showIntensity, this);
		FunctionAreaMenu_addCommand (menu, U"Intensity settings...", 0,
		        menu_cb_intensitySettings, this);
		FunctionAreaMenu_addCommand (
		        menu, U"- Query intensity:", 0, nullptr, this);
		FunctionAreaMenu_addCommand (menu, U"Intensity listing", 1,
		        INFO_DATA__intensityListing, this);
		FunctionAreaMenu_addCommand (menu, U"Get intensity",
		        GuiMenu_F8 | GuiMenu_DEPTH_1, QUERY_DATA_FOR_REAL__getIntensity,
		        this);
		FunctionAreaMenu_addCommand (menu, U"Get minimum intensity",
		        GuiMenu_F8 | GuiMenu_COMMAND | GuiMenu_DEPTH_1,
		        QUERY_DATA_FOR_REAL__getMinimumIntensity, this);
		FunctionAreaMenu_addCommand (menu, U"Get maximum intensity",
		        GuiMenu_F8 | GuiMenu_SHIFT | GuiMenu_DEPTH_1,
		        QUERY_DATA_FOR_REAL__getMaximumIntensity, this);
		FunctionAreaMenu_addCommand (
		        menu, U"- Draw intensity to picture window:", 0, nullptr, this);
		FunctionAreaMenu_addCommand (menu, U"Draw visible intensity contour...",
		        1, menu_cb_drawVisibleIntensityContour, this);
		FunctionAreaMenu_addCommand (menu,
		        U"- Extract intensity to objects window:", 0, nullptr, this);
		FunctionAreaMenu_addCommand (menu, U"Extract visible intensity contour",
		        1, CONVERT_DATA_TO_ONE__ExtractVisibleIntensityContour, this);
	}
	...
	if (our v_hasPulses ()) {
		...
		our pulsesToggle = FunctionAreaMenu_addCommand (menu, U"Show pulses",
		        GuiMenu_CHECKBUTTON |
		                (instancePref_pulses_show () ? GuiMenu_TOGGLE_ON : 0),
		        menu_cb_showPulses, this);
		FunctionAreaMenu_addCommand (menu, U"Advanced pulses settings...", 0,
		        menu_cb_advancedPulsesSettings, this);
		FunctionAreaMenu_addCommand (
		        menu, U"- Query pulses:", 0, nullptr, this);
		FunctionAreaMenu_addCommand (
		        menu, U"Voice report", 1, INFO_DATA__voiceReport, this);
		FunctionAreaMenu_addCommand (
		        menu, U"Pulse listing", 1, INFO_DATA__pulseListing, this);
		/*
		 FunctionAreaMenu_addCommand (menu, U"Get jitter (local)", 0,
		 cb_getJitter_local, this); FunctionAreaMenu_addCommand (menu, U"Get
		 jitter (local, absolute)", 0, cb_getJitter_local_absolute, this);
		 FunctionAreaMenu_addCommand (menu, U"Get jitter (rap)", 0,
		 cb_getJitter_rap, this); FunctionAreaMenu_addCommand (menu, U"Get
		 jitter (ppq5)", 0, cb_getJitter_ppq5, this);
		 FunctionAreaMenu_addCommand (menu, U"Get jitter (ddp)", 0,
		 cb_getJitter_ddp, this); FunctionAreaMenu_addCommand (menu, U"Get
		 shimmer (local)", 0, cb_getShimmer_local, this);
		 FunctionAreaMenu_addCommand (menu, U"Get shimmer (local_dB)", 0,
		 cb_getShimmer_local_dB, this); FunctionAreaMenu_addCommand (menu, U"Get
		 shimmer (apq3)", 0, cb_getShimmer_apq3, this);
		 FunctionAreaMenu_addCommand (menu, U"Get shimmer (apq5)", 0,
		 cb_getShimmer_apq5, this); FunctionAreaMenu_addCommand (menu, U"Get
		 shimmer (apq11)", 0, cb_getShimmer_apq11, this);
		 FunctionAreaMenu_addCommand (menu, U"Get shimmer (dda)", 0,
		 cb_getShimmer_dda, this);
		 */
		FunctionAreaMenu_addCommand (
		        menu, U"- Draw pulses to picture window:", 0, nullptr, this);
		FunctionAreaMenu_addCommand (menu, U"Draw visible pulses...", 1,
		        menu_cb_drawVisiblePulses, this);
		FunctionAreaMenu_addCommand (
		        menu, U"- Extract pulses to objects window:", 0, nullptr, this);
...
static void SoundAnalysisArea_v_draw_analysis (SoundAnalysisArea me) {
	/*
	 d_pitch may not exist yet (if shown at all, it may be going to be
	 created in tryToHavePitch (), and even if that fails the user should see
	 what the pitch settings are). So we use a dummy object.
	 */
	const double pitchFloor_hidden = Function_convertStandardToSpecialUnit (
	        Thing_dummyObject (Pitch), my instancePref_pitch_floor (),
	        Pitch_LEVEL_FREQUENCY, (int) my instancePref_pitch_unit ());
	const double pitchCeiling_hidden = Function_convertStandardToSpecialUnit (
	        Thing_dummyObject (Pitch), my instancePref_pitch_ceiling (),
	        Pitch_LEVEL_FREQUENCY, (int) my instancePref_pitch_unit ());
	const double pitchFloor_overt = Function_convertToNonlogarithmic (
	        Thing_dummyObject (Pitch), pitchFloor_hidden, Pitch_LEVEL_FREQUENCY,
	        (int) my instancePref_pitch_unit ());
	const double pitchCeiling_overt = Function_convertToNonlogarithmic (
	        Thing_dummyObject (Pitch), pitchCeiling_hidden,
	        Pitch_LEVEL_FREQUENCY, (int) my instancePref_pitch_unit ());
	const double pitchViewFrom_overt =
	        (my instancePref_pitch_viewFrom () < my instancePref_pitch_viewTo ()
	                        ? my instancePref_pitch_viewFrom ()
	...
	const double pitchViewTo_overt =
	        (my instancePref_pitch_viewFrom () < my instancePref_pitch_viewTo ()
	                        ? my instancePref_pitch_viewTo ()
	                        : pitchCeiling_overt);
	const double pitchViewFrom_hidden =
	        Function_isUnitLogarithmic (Thing_dummyObject (Pitch),
	                Pitch_LEVEL_FREQUENCY, (int) my instancePref_pitch_unit ())
	                ? log10 (pitchViewFrom_overt)
	                : pitchViewFrom_overt;
	const double pitchViewTo_hidden =
	        Function_isUnitLogarithmic (Thing_dummyObject (Pitch),
	                Pitch_LEVEL_FREQUENCY, (int) my instancePref_pitch_unit ())
	                ? log10 (pitchViewTo_overt)
	                : pitchViewTo_overt;

	const double pitchFloor_hidden_semitones440 =
	        Function_convertStandardToSpecialUnit (Thing_dummyObject (Pitch),
	                my instancePref_pitch_floor (), Pitch_LEVEL_FREQUENCY,
	                (int) kPitch_unit::SEMITONES_440);
	const double pitchCeiling_hidden_semitones440 =
	        Function_convertStandardToSpecialUnit (Thing_dummyObject (Pitch),
	                my instancePref_pitch_ceiling (), Pitch_LEVEL_FREQUENCY,
	                (int) kPitch_unit::SEMITONES_440);
	const double pitchFloor_overt_semitones440 =
	        Function_convertToNonlogarithmic (Thing_dummyObject (Pitch),
	                pitchFloor_hidden_semitones440, Pitch_LEVEL_FREQUENCY,
	...
	if (my endWindow () - my startWindow () >
		...
		Graphics_setTextAlignment (
		        my graphics (), Graphics_CENTRE, Graphics_HALF);
		Graphics_text (my graphics (), 0.5, 0.67,
		        U"(To see the analyses, zoom in to at most ",
		        Melder_half (my instancePref_longestAnalysis ()), U" seconds,");
		Graphics_text (my graphics (), 0.5, 0.33,
		        U"or raise the \"longest analysis\" setting with \"Show "
		        U"analyses\" in the Analyses menu.)");
	...
	if (my instancePref_pitch_show () && my d_pitch) {
		const double periodsPerAnalysisWindow =
		        (my instancePref_pitch_method () ==
		                                kSoundAnalysisArea_pitch_analysisMethod::
		                                        AUTOCORRELATION
		                        ? 3.0
		                        : 1.0);
		const double greatestNonUndersamplingTimeStep =
		        0.5 * periodsPerAnalysisWindow / my instancePref_pitch_floor ();
		...
		const integer numberOfVisiblePitchPoints =
		        (integer) ((my endWindow () - my startWindow ()) /
		                   timeStep);   // BUG: why round down?
		Graphics_setColour (my graphics (), Melder_CYAN);
		Graphics_setLineWidth (my graphics (), 3.0);
		if ((my instancePref_pitch_drawingMethod () ==
		                    kSoundAnalysisArea_pitch_drawingMethod::AUTOMATIC &&
		            (undersampled || numberOfVisiblePitchPoints < 101)) ||
		        my instancePref_pitch_drawingMethod () ==
		                kSoundAnalysisArea_pitch_drawingMethod::SPECKLE) {
			Pitch_drawInside (my d_pitch.get (), my graphics (),
			        my startWindow (), my endWindow (), pitchViewFrom_overt,
			        pitchViewTo_overt, 2, my instancePref_pitch_unit ());
		}
		if ((my instancePref_pitch_drawingMethod () ==
		                    kSoundAnalysisArea_pitch_drawingMethod::AUTOMATIC &&
		            !undersampled) ||
		        my instancePref_pitch_drawingMethod () ==
		                kSoundAnalysisArea_pitch_drawingMethod::CURVE) {
			Pitch_drawInside (my d_pitch.get (), my graphics (),
			        my startWindow (), my endWindow (), pitchViewFrom_overt,
			        pitchViewTo_overt, false, my instancePref_pitch_unit ());
		}
		...
		if ((my instancePref_pitch_drawingMethod () ==
			...
			Pitch_drawInside (my d_pitch.get (), my graphics (),
			        my startWindow (), my endWindow (), pitchViewFrom_overt,
			        pitchViewTo_overt, 1, my instancePref_pitch_unit ());
		}
		if ((my instancePref_pitch_drawingMethod () ==
		                    kSoundAnalysisArea_pitch_drawingMethod::AUTOMATIC &&
		            !undersampled) ||
	...
	if (my instancePref_intensity_show ()) tryToHaveIntensity (me);
	if (my instancePref_intensity_show () && my d_intensity) {
		Graphics_setColour (my graphics (), my instancePref_spectrogram_show ()
		                                            ? Melder_YELLOW
		                                            : Melder_LIME);
		Graphics_setLineWidth (my graphics (),
		        my instancePref_spectrogram_show () ? 1.0 : 3.0);
		Intensity_drawInside (my d_intensity.get (), my graphics (),
		        my startWindow (), my endWindow (),
		        my instancePref_intensity_viewFrom (),
		        my instancePref_intensity_viewTo ());
		Graphics_setLineWidth (my graphics (), 1.0);
		Graphics_setColour (my graphics (), Melder_BLACK);
	}
	...
	if (my instancePref_pitchGrid_show () && my d_pitch) {
		Graphics_setWindow (my graphics (), my startWindow (), my endWindow (),
		        pitchViewFrom_hidden_semitones440,
		        pitchViewTo_hidden_semitones440);
		...
		// Draw dynamic pitch grid centered on cursor position
		// Grid lines are at cursor ± 0.5 semitones (same as tier notes)
		if (isdefined (my d_pitchGrid_cursor)) {
			double gridBottom = my d_pitchGrid_cursor - 0.5;
			double gridTop = my d_pitchGrid_cursor + 0.5;

			// Draw the two horizontal lines defining the grid box
			if (gridBottom >= pitchViewFrom_hidden_semitones440 &&
			        gridBottom <= pitchViewTo_hidden_semitones440) {
				Graphics_line (my graphics (), my startWindow (), gridBottom,
				        my endWindow (), gridBottom);
			}
			if (gridTop >= pitchViewFrom_hidden_semitones440 &&
			        gridTop <= pitchViewTo_hidden_semitones440) {
				Graphics_line (my graphics (), my startWindow (), gridTop,
				        my endWindow (), gridTop);
			}
		}
	}
	...
	if (!my tierNotesData.empty ()) {
		...
		// Always use full semitone height (1 semitone total)
		// Center the display on the actual pitch value (with decimals)
		double halfNoteHeight = 0.5;
		for (integer i = 0; i < my tierNotesData.size (); ++i) {
			// Use the actual pitch value as center, extend ±0.5 semitones
			// e.g., 60.65 → bottom: 60.15, top: 61.15
			double pitchValue = my tierNotesData.at (i).at (2);
			double bottom = pitchValue - 69 - halfNoteHeight;
			double top = pitchValue - 69 + halfNoteHeight;
			// if (my tierNotesData.at(i).at(2) == -1) {
			// 	bottom = pitchViewFrom_hidden_semitones440;
			// 	top = pitchViewTo_hidden_semitones440;
			// }
			if (my tierNotesData.at (i).at (2) == -2) continue;
			Graphics_fillRectangle_transparent (my graphics (),
			        my tierNotesData.at (i).at (0),
			        my tierNotesData.at (i).at (1), bottom, top);
		}
		Graphics_setLineType (my graphics (), Graphics_DRAWN);
	}
	...
	if (my instancePref_pitch_show ()) {
		...
		double pitchCursor_semitones440_overt = undefined,
		       pitchCursor_semitones440_hidden = undefined;
		Graphics_setWindow (my graphics (), my startWindow (), my endWindow (),
		        pitchViewFrom_hidden, pitchViewTo_hidden);
		Graphics_setColour (my graphics (), 1.2 * Melder_BLUE);
		if (my d_pitch) {
			if (my startSelection () == my endSelection ()) {
				pitchCursor_hidden = Pitch_getValueAtTime (my d_pitch.get (),
				        my startSelection (), my instancePref_pitch_unit (), 1);
				pitchCursor_semitones440_hidden = Pitch_getValueAtTime (
				        my d_pitch.get (), my startSelection (),
				        kPitch_unit::SEMITONES_440, 1);
			} else {
				pitchCursor_hidden = Pitch_getMean (my d_pitch.get (),
				        my startSelection (), my endSelection (),
				        my instancePref_pitch_unit ());
				pitchCursor_semitones440_hidden =
				        Pitch_getMean (my d_pitch.get (), my startSelection (),
				                my endSelection (), kPitch_unit::SEMITONES_440);
			}
			pitchCursor_overt = Function_convertToNonlogarithmic (
			        my d_pitch.get (), pitchCursor_hidden,
			        Pitch_LEVEL_FREQUENCY, (int) my instancePref_pitch_unit ());
			pitchCursor_semitones440_overt = Function_convertToNonlogarithmic (
			        my d_pitch.get (), pitchCursor_semitones440_hidden,
			        Pitch_LEVEL_FREQUENCY, (int) kPitch_unit::SEMITONES_440);
			if (isdefined (pitchCursor_hidden)) {
				Graphics_setColour (
				        my graphics (), MelderColour (0.6, 0.0, 0.4));
				Graphics_setTextAlignment (
				        my graphics (), Graphics_LEFT, Graphics_HALF);
				if (kPitch_unit::HERTZ == my instancePref_pitch_unit () ||
				        kPitch_unit::HERTZ_LOGARITHMIC ==
				                my instancePref_pitch_unit ())
					Graphics_text (my graphics (), my endWindow (),
					        pitchCursor_hidden,
					        Melder_float (Melder_half (pitchCursor_overt)),
					        U" ",
					        Function_getUnitText (my d_pitch.get (),
					                Pitch_LEVEL_FREQUENCY,
					                (int) my instancePref_pitch_unit (),
					                Function_UNIT_TEXT_SHORT |
					        ...
					        Melder_float (Melder_half (
					                pitchCursor_semitones440_overt + 69)),
					        U" ",
					        Function_getUnitText (my d_pitch.get (),
					                Pitch_LEVEL_FREQUENCY,
					                (int) kPitch_unit::SEMITONES_440,
			...
			if (isundef (pitchCursor_hidden) ||
			        Graphics_dyWCtoMM (my graphics (),
			                pitchCursor_hidden - pitchViewFrom_hidden) > 4.0) {
				Graphics_setTextAlignment (
				        my graphics (), Graphics_LEFT, Graphics_HALF);
				Graphics_text (my graphics (), my endWindow (),
				        pitchViewFrom_hidden -
				                Graphics_dyMMtoWC (my graphics (), 0.5),
			...
			if (isundef (pitchCursor_hidden) ||
			        Graphics_dyWCtoMM (my graphics (),
			                pitchViewTo_hidden - pitchCursor_hidden) > 4.0) {
				Graphics_setTextAlignment (
				        my graphics (), Graphics_LEFT, Graphics_HALF);
				Graphics_text (my graphics (), my endWindow (),
				        pitchViewTo_hidden,
			...
			Graphics_text (my graphics (),
			        0.5 * (my startWindow () + my endWindow ()),
			        0.5 * (pitchViewFrom_hidden + pitchViewTo_hidden),
			        U"(Cannot show pitch contour. Zoom out or change "
			        U"bottom of "
			        U"pitch range in pitch settings.)");
			Graphics_setFontSize (my graphics (), 12);
		}
		Graphics_setColour (my graphics (), Melder_BLACK);
	}
	if (my instancePref_intensity_show ()) {
		...
		if (!my instancePref_pitch_show ()) {
			...
			vert = Graphics_HALF;
			y = my endWindow ();
		} else if (!my instancePref_spectrogram_show () &&
			...
			textColour =
			        (my instancePref_spectrogram_show () ? Melder_LIME
			                                             : 1.2 * Melder_GREEN);
			hor = Graphics_RIGHT;
			vert = -1;
			y = my endWindow ();
		}
		if (my instancePref_intensity_viewTo () >
		        my instancePref_intensity_viewFrom ()) {
			Graphics_setWindow (my graphics (), my startWindow (),
			        my endWindow (), my instancePref_intensity_viewFrom (),
			        my instancePref_intensity_viewTo ());
			if (my d_intensity) {
				if (my startSelection () == my endSelection ()) {
					intensityCursor = Vector_getValueAtX (my d_intensity.get (),
					        my startSelection (), Vector_CHANNEL_1,
					        kVector_valueInterpolation ::LINEAR);
				} else {
					intensityCursor = Intensity_getAverage (
					        my d_intensity.get (), my startSelection (),
					        my endSelection (),
					        (int) my instancePref_intensity_averagingMethod ());
				}
			}
			Graphics_setColour (my graphics (), textColour);
			const bool intensityCursorVisible =
			        (isdefined (intensityCursor) &&
			                intensityCursor >
			                        my instancePref_intensity_viewFrom () &&
			                intensityCursor <
			                        my instancePref_intensity_viewTo ());
			...
			if (!intensityCursorVisible ||
			        Graphics_dyWCtoMM (my graphics (),
			                intensityCursor -
			                        my instancePref_intensity_viewFrom ()) >
			                5.0) {
				Graphics_setTextAlignment (my graphics (), hor,
				        vert == -1 ? Graphics_BOTTOM : Graphics_HALF);
				Graphics_text (my graphics (), y,
				        my instancePref_intensity_viewFrom () -
				                Graphics_dyMMtoWC (my graphics (), 0.5),
				        Melder_float (Melder_half (
				                my instancePref_intensity_viewFrom ())),
			...
			if (!intensityCursorVisible ||
			        Graphics_dyWCtoMM (my graphics (),
			                my instancePref_intensity_viewTo () -
			                        intensityCursor) > 5.0) {
				Graphics_setTextAlignment (my graphics (), hor,
				        vert == -1 ? Graphics_TOP : Graphics_HALF);
				Graphics_text (my graphics (), y,
				        my instancePref_intensity_viewTo (),
				        Melder_float (Melder_half (
				                my instancePref_intensity_viewTo ())),
	...
	if (my instancePref_spectrogram_show () ||
	        my instancePref_formant_show ()) {
		const bool frequencyCursorVisible =
		...
		if (my instancePref_cqt_show () &&
			...
			// @lixu 临时关闭计算-NO
			Graphics_setWindow (my graphics (), my startWindow (),
			        my endWindow (), 0.0, 1.0);
		} else {
			// For regular spectrogram, use linear scale
			Graphics_setWindow (my graphics (), my startWindow (),
			        my endWindow (), my instancePref_spectrogram_viewFrom (),
			        my instancePref_spectrogram_viewTo ());
		}
		/*
		 Range marks.
		...
		if (my instancePref_cqt_show () &&
			...
			if (!frequencyCursorVisible || cursorLogPos > 0.1) {
				Graphics_setTextAlignment (
				        my graphics (), Graphics_RIGHT, Graphics_HALF);
				...
				Graphics_text (my graphics (), my startWindow (),
				        ...
				        midiNoteMin, U" - ", Melder_float (Melder_half (fmin)),
				        U" Hz");
			}
			...
			if (!frequencyCursorVisible || cursorLogPos < 0.9) {
				...
				Graphics_text (my graphics (), my startWindow (), 1.0,
				        midiNoteMax, U" - ", Melder_float (Melder_half (fmax)),
				        U" Hz");
			}

			// Calculate semitone range for note-level grid
			const double logFmin = log (fmin);
			const double logFmax = log (fmax);
			const double logSemitone = log (pow (2.0, 1.0 / 12.0));
			const double logOctave = log (2.0);

			// Draw note-level grid lines (short lines on left, 1% width)
			const double noteLineWidth =
			        0.01 * (my endWindow () - my startWindow ());
			...
			for (int i = iStart; i <= iEnd; ++i) {
				...
				double f = exp (
				        logF);   // You had this line twice; one needs to go

				if (f > fmin && f < fmax) {
					const double logPos =
					        (logF - logFmin) / (logFmax - logFmin);
					const bool isOctave = (fmod (logF / logOctave, 1.0) < 0.01);

					Graphics_setLineType (my graphics (), Graphics_DOTTED);

					if (isOctave) {
						// Octave lines: full width, sky blue for good contrast
						// against grayscale spectrogram
						Graphics_setColour (my graphics (),
					...
					Graphics_setColour (
					        my graphics (), DataGuiColour_NONEDITABLE);
				}
			}
#if cocoa
			double mediaTimeEnd = CACurrentMediaTime () * 1000;
		...
		if (frequencyCursorVisible) {
			if (my instancePref_cqt_show () &&
				...
				const double y = frequencyToLogPosition (
				        my d_spectrogram_cursor, fmin, fmax);
				const integer midiNote =
				        frequencyToMidiNoteInteger (my d_spectrogram_cursor);

				// Left side label only
				Graphics_setTextAlignment (
				        my graphics (), Graphics_RIGHT, Graphics_HALF);
				Graphics_text (my graphics (), my startWindow (), y, midiNote,
				        U" - ",
				        Melder_float (Melder_half (my d_spectrogram_cursor)),
				        U" Hz");

				Graphics_line (my graphics (), my startWindow (), y,
				        my endWindow (), y);
			} else {
				// For linear mode, use frequency directly
				const double y = my d_spectrogram_cursor;
				Graphics_setTextAlignment (
				        my graphics (), Graphics_RIGHT, Graphics_HALF);
				Graphics_text (my graphics (), my startWindow (), y,
				        Melder_float (Melder_half (y)), U" Hz");
				Graphics_line (my graphics (), my startWindow (), y,
				        my endWindow (), y);
			}
		}
		/*
		 if (our startSelection >= our startWindow && our startSelection <=
		 our endWindow) Graphics_line (our graphics, our startSelection, our
		 p_spectrogram_viewFrom, our startSelection, our
		 p_spectrogram_viewTo); if (our endSelection > our startWindow && our
		 endSelection < our endWindow && our endSelection != our
		 startSelection) Graphics_line (our graphics, our endSelection, our
		 p_spectrogram_viewFrom, our endSelection, our
		 p_spectrogram_viewTo);*/
		Graphics_setLineType (my graphics (), Graphics_DRAWN);
	}
	if (my instancePref_pitchGrid_show () && my d_pitch) {
		/*
		...
		// Calculate quantized pitch value (same as what would be written to
		// TextGrid)
		double pitchValue = my d_pitchGrid_cursor + 69.0;
		double resolution = my tierNotesFrequencyResolution;
		double quantizedPitch = round (pitchValue / resolution) * resolution;

		// Determine which note name to display based on quantized value
		// e.g., 49.50 and below -> 49 (C#3), 50.50 and above -> 51 (D#3)
		const integer midiNote = Melder_iround (quantizedPitch);

		// Display format: "60.65 D3 | re" or just "60.65" if note names
		// disabled
		if (my instancePref_pitchGrid_showNoteNames ()) {
			conststring32 noteName = midiNoteToNoteName (
			        midiNote, my instancePref_pitchGrid_key ());
			conststring32 solfegeName = midiNoteToSolfege (
			        midiNote, my instancePref_pitchGrid_key ());
			Graphics_text (my graphics (), my endWindow (),
			        my d_pitchGrid_cursor, Melder_fixed (quantizedPitch, 2),
			        U" ", noteName, U" | ", solfegeName);
		} else {
			Graphics_text (my graphics (), my endWindow (),
			        my d_pitchGrid_cursor, Melder_fixed (quantizedPitch, 2));
		}
...
#if cocoa
	double mediaTimeStart =
	        CACurrentMediaTime () *
	        1000;   // CACurrentMediaTime() * 1000; mach_absolute_time()
#endif
...
void structSoundAnalysisArea ::v_draw_analysis_formants () {
	if (our instancePref_formant_show ()) tryToHaveFormants (this);
	if (our instancePref_formant_show () && our d_formant) {
		Graphics_setSpeckleSize (
		        our graphics (), our instancePref_formant_dotSize ());
		Formant_drawSpeckles_inside (our d_formant.get (), our graphics (),
		        our startWindow (), our endWindow (),
		        our instancePref_spectrogram_viewFrom (),
		        our instancePref_spectrogram_viewTo (),
		        our instancePref_formant_dynamicRange (), Melder_RED,
		        Melder_PINK, true);
...
void structSoundAnalysisArea ::v_draw_analysis_pulses () {
	if (our instancePref_pulses_show ()) tryToHavePulses (this);
	if (our instancePref_pulses_show () &&
		...
		Graphics_setWindow (our graphics (), our startWindow (),
		        our endWindow (), -1.0, 1.0);
		Graphics_setColour (our graphics (),
		        our editable () ? DataGuiColour_EDITABLE : Melder_SILVER);
...
void structSoundAnalysisArea ::v9_repairPreferences () {
	if (!(our instancePref_pitch_floor () <
	            our instancePref_pitch_ceiling ())) {   // NaN-safe test
		our setInstancePref_pitch_floor (
		        Melder_atof (our default_pitch_floor ()));
		our setInstancePref_pitch_ceiling (
		        Melder_atof (our default_pitch_ceiling ()));
		our setInstancePref_pitch_unit (kPitch_unit::HERTZ);
	}
	if (!(our instancePref_spectrogram_viewFrom () <
	            our instancePref_spectrogram_viewTo ())) {   // NaN-safe
		// test
		our setInstancePref_spectrogram_viewFrom (
		        Melder_atof (our default_spectrogram_viewFrom ()));
		our setInstancePref_spectrogram_viewTo (
		        Melder_atof (our default_spectrogram_viewTo ()));
	}
	if (!(our instancePref_intensity_viewFrom () <
	            our instancePref_intensity_viewTo ())) {   // NaN-safe test
		our setInstancePref_intensity_viewFrom (
		        Melder_atof (our default_intensity_viewFrom ()));
		our setInstancePref_intensity_viewTo (
		        Melder_atof (our default_intensity_viewTo ()));
	}
	if (our instancePref_log1_toLogFile () == false &&
	        our instancePref_log1_toInfoWindow () == false) {
		our setInstancePref_log1_toLogFile (true);
		our setInstancePref_log1_toInfoWindow (true);
	}
	if (our instancePref_log2_toLogFile () == false &&
	...
	if ((int) our instancePref_pitchGrid_key () < 1 ||
	        (int) our instancePref_pitchGrid_key () > 12) {
		our setInstancePref_pitchGrid_key (
		        kSoundAnalysisArea_pitchGrid_key::C_MAJOR);
	}
	...
	// be affected?
	if (!our v_hasPitch ()) our setInstancePref_pitch_show (false);
	if (!our v_hasIntensity ()) our setInstancePref_intensity_show (false);
	if (!our v_hasFormants ()) our setInstancePref_formant_show (false);
	if (!our v_hasPulses ()) our setInstancePref_pulses_show (false);
