...
static void checkTierSelection (
        constTextGridArea me, conststring32 verbPhrase) {
	if (my selectedTier < 1 || my selectedTier > my textGrid ()->tiers->size)
		Melder_throw (U"To ", verbPhrase,
		        U", first select a tier by clicking anywhere inside it.");
}
static integer getSelectedInterval (constTextGridArea me) {
	Melder_assert (my selectedTier >= 1 ||
	               my selectedTier <= my textGrid ()->tiers->size);
	const IntervalTier tier =
	        (IntervalTier) my textGrid () -> tiers->at[my selectedTier];
...
static integer getSelectedPoint (constTextGridArea me) {
	Melder_assert (my selectedTier >= 1 ||
	               my selectedTier <= my textGrid ()->tiers->size);
	const TextTier tier =
	        (TextTier) my textGrid () -> tiers->at[my selectedTier];
		...
		*out_tmax = interval->xmax;
		...
		if (n == 0) {
			...
			*out_tmax = my tmax ();
			...
			integer ipointleft =
			        AnyTier_timeToLowIndex (textTier->asAnyTier (), t);
			*out_tmin =
			        (ipointleft == 0 ? my tmin ()
			                         : textTier->points.at[ipointleft]->number);
			*out_tmax = (ipointleft == n
			                     ? my tmax ()
			                     : textTier->points.at[ipointleft + 1]->number);
		}
	}
...
static void insertBoundaryOrPoint (TextGridArea me, integer itier, double t1,
        double t2, bool insertSecond) {
	const integer numberOfTiers = my textGrid () -> tiers->size;
	if (itier < 1 || itier > numberOfTiers)
		Melder_throw (U"No tier ", itier, U".");
	IntervalTier intervalTier;
	TextTier textTier;
	AnyTextGridTier_identifyClass (
	        my textGrid ()->tiers->at[itier], &intervalTier, &textTier);
	Melder_assert (t1 <= t2);
	...
	if (intervalTier) {
		...
		if (t1 == t2 && t1IsABoundary)
			Melder_throw (U"Cannot add a boundary at ", Melder_fixed (t1, 6),
			        U" seconds, because there is already a boundary there.");
		if (t1IsABoundary && t2IsABoundary)
			Melder_throw (U"Cannot add boundaries at ", Melder_fixed (t1, 6),
			        U" and ", Melder_fixed (t2, 6),
			        U" seconds, because there are already boundaries there.");
		const integer iinterval = IntervalTier_timeToIndex (intervalTier, t1);
		const integer iinterval2 =
		        t1 == t2 ? iinterval
		                 : IntervalTier_timeToIndex (intervalTier, t2);
		if (iinterval == 0 || iinterval2 == 0)
			Melder_throw (U"The selection is outside the time domain of the "
			              U"intervals.");
		const integer correctedIinterval2 =
		        (t2IsABoundary && iinterval2 == intervalTier->intervals.size
		                        ? iinterval2 + 1
		                        : iinterval2);
		if (correctedIinterval2 > iinterval + 1 ||
		        (correctedIinterval2 > iinterval && !t2IsABoundary))
		...
		if (itier == my selectedTier) {
			/*
			    Divide up the label text into left, mid and right, depending on
			   where the text selection is.
			*/
			integer left, right;
			autostring32 text = GuiText_getStringAndSelectionPosition (
			        my functionEditor ()->textArea, &left, &right);
			const bool wholeTextIsSelected =
			        (right - left == Melder_length (text.get ()));
			rightNewInterval = TextInterval_create (
			        t2, interval->xmax, text.get () + right);
			text[right] = U'\0';
		...
		if (t1IsABoundary) {
			...
			*/
			if (interval->xmin != t1)
				Melder_fatal (U"Boundary unequal: ", interval->xmin,
				        U" versus ", t1, U".");
			interval->xmax = t2;
			TextInterval_setText (
			        interval, Melder_cat (interval->text.get (),
			                          midNewInterval->text.get ()));
		} else if (t2IsABoundary) {
			/*
			    Merge mid and right interval.
			*/
			if (interval->xmax != t2)
				Melder_fatal (U"Boundary unequal: ", interval->xmax,
				        U" versus ", t2, U".");
			interval->xmax = t1;
			Melder_assert (rightNewInterval->xmin == t2);
			Melder_assert (rightNewInterval->xmax == t2);
			rightNewInterval->xmin = t1;
			TextInterval_setText (rightNewInterval.get (),
			        Melder_cat (midNewInterval->text.get (),
			                rightNewInterval->text.get ()));
		} else {
			interval->xmax = t1;
			if (t1 != t2)
			...
			*/
			...
			for (integer jtier = 1; jtier <= numberOfTiers; jtier++) {
				if (jtier != itier) {
					...
					timeToInterval (
					        me, t1, jtier, &startInterval, &endInterval);
					if (startInterval > tlast) tlast = startInterval;
				}
			}
			if (tlast > interval->xmin && tlast < t1) {
				autoTextInterval newInterval =
				        TextInterval_create (tlast, t1, U"");
				interval->xmax = tlast;
		...
		if (AnyTier_hasPoint (textTier->asAnyTier (), t1))
			Melder_throw (U"Cannot add a point at ", Melder_fixed (t1, 6),
			        U" seconds, because there is already a point there.");
...
static std::vector<std::vector<double>> getSelectedTierData (TextGridArea me) {
	...
	if (!isIntervalTier) return tierNotesData;
	...
	for (integer iinterval = 1; iinterval <= numberOfIntervals; iinterval++) {
		...
		/* mutable clip */ double startInterval = interval->xmin,
		                          endInterval = interval->xmax;
		Melder_clipLeft (my tmin (), &startInterval);
		Melder_clipRight (&endInterval, my tmax ());
		if (startInterval >= endInterval) continue;

		if (interval->text && endInterval >= my startWindow () &&
		        startInterval <= my endWindow ()) {
...
void structTextGridArea ::v1_info () {
	...
	MelderInfo_writeLine (
	        U"TextGrid uses text styles: ", our instancePref_useTextStyles ());
	MelderInfo_writeLine (
	        U"TextGrid font size: ", our instancePref_fontSize ());
	MelderInfo_writeLine (
	        U"TextGrid alignment: ", kGraphics_horizontalAlignment_getText (
	                                         our instancePref_alignment ()));
}
...
void structTextGridArea ::v_specializedHighlightBackground () const {
	...
	for (integer itier = 1; itier <= numberOfTiers; itier++) {
		...
		if (anyTier->classInfo != classIntervalTier) continue;
		IntervalTier tier = (IntervalTier) anyTier;
		/*
		    Highlight interval: yellow (selected) or green (matching label).
		*/
		const integer selectedInterval =
		        (itier == our selectedTier ? getSelectedInterval (this) : 0);
		const integer numberOfIntervals = tier->intervals.size;
		for (integer iinterval = 1; iinterval <= numberOfIntervals;
		        iinterval++) {
			const TextInterval interval = tier->intervals.at[iinterval];
			/* mutable clip */ double startInterval = interval->xmin,
			                          endInterval = interval->xmax;
			if (endInterval > our startWindow () &&
			        startInterval < our endWindow ()) {   // interval visible?
				const bool intervalIsSelected = (iinterval == selectedInterval);
				const bool labelDoesMatch = Melder_stringMatchesCriterion (
				        interval->text.get (), our instancePref_greenMethod (),
				        our instancePref_greenString (), true);
				...
				if (labelDoesMatch) {
					...
					Graphics_fillRectangle (our graphics (), startInterval,
					        endInterval, bottom, top);
				}
				if (intervalIsSelected) {
					Graphics_setColour (our graphics (), Melder_YELLOW);
					Graphics_fillRectangle (our graphics (),
					        labelDoesMatch
					                ? 0.85 * startInterval + 0.15 * endInterval
					                : startInterval,
					        labelDoesMatch
					                ? 0.15 * startInterval + 0.85 * endInterval
					                : endInterval,
					        labelDoesMatch ? 0.85 * bottom + 0.15 * top
					                       : bottom,
...
static void do_drawIntervalTier (
        TextGridArea me, IntervalTier tier, integer itier) {
	...
	Graphics_setPercentSignIsItalic (
	        my graphics (), my instancePref_useTextStyles ());
	Graphics_setNumberSignIsBold (
	        my graphics (), my instancePref_useTextStyles ());
	Graphics_setCircumflexIsSuperscript (
	        my graphics (), my instancePref_useTextStyles ());
	Graphics_setUnderscoreIsSubscript (
	        my graphics (), my instancePref_useTextStyles ());

	const integer selectedInterval =
	        (itier == my selectedTier ? getSelectedInterval (me) : 0);
	const integer numberOfIntervals = tier->intervals.size;
	...
	*/
	if (my startSelection () == my endSelection () &&
		...
		for (integer iinterval = 2; iinterval <= numberOfIntervals;
		        iinterval++) {
			const TextInterval interval = tier->intervals.at[iinterval];
		...
		if (!cursorAtBoundary) {
			...
			Graphics_setLineWidth (
			        my graphics (), platformUsesAntiAliasing ? 6.0 : 5.0);
			Graphics_line (my graphics (), my startSelection (), 0.0,
			        my startSelection (), 1.0);
			Graphics_setLineWidth (my graphics (), 1.0);
			if (my editable ()) {
				Graphics_setColour (my graphics (), Melder_BLUE);
				Graphics_circle_mm (
				        my graphics (), my startSelection (), 1.0 - dy, 3.0);
			}
		}
	}

	Graphics_setTextAlignment (
	        my graphics (), my instancePref_alignment (), Graphics_HALF);
	for (integer iinterval = 1; iinterval <= numberOfIntervals; iinterval++) {
		const TextInterval interval = tier->intervals.at[iinterval];
		/* mutable clip */ double startInterval = interval->xmin,
		                          endInterval = interval->xmax;
		...
		/*
		    Draw left boundary.
		*/
		if (startInterval >= my startWindow () &&
		        startInterval <= my endWindow () && iinterval > 1) {
			const bool boundaryIsSelected =
			        (my selectedTier == itier &&
			                startInterval == my startSelection ());
			Graphics_setColour (my graphics (),
			        DataGui_defaultForegroundColour (me, boundaryIsSelected));
			Graphics_setLineWidth (
			        my graphics (), platformUsesAntiAliasing ? 6.0 : 5.0);
			Graphics_line (
			        my graphics (), startInterval, 0.0, startInterval, 1.0);

			/*
			    Draw a red dotted line for each boundary
			*/
			if (my instancePref_allBoundaryLines_show () &&
			        itier == my selectedTier) {
				Graphics_setColour (my graphics (), Melder_RED);
				Graphics_setLineType (my graphics (), Graphics_DOTTED);
				Graphics_setLineWidth (my graphics (), 1.0);
				Graphics_line (my graphics (), startInterval, 0.0,
				        startInterval, 50); /* "50" just for temp use */
			...
			*/
			if (startInterval == my startSelection ()) {
				...
				Graphics_setLineWidth (
				        my graphics (), platformUsesAntiAliasing ? 2.0 : 1.0);
				Graphics_line (
				        my graphics (), startInterval, 0.0, startInterval, 1.0);
			}
		}
		...
		*/
		if (interval->text && endInterval >= my startWindow () &&
			...
			const double t2 = std::min (my endWindow (), endInterval);
			Graphics_setColour (my graphics (),
			        DataGui_defaultForegroundColour (me, intervalIsSelected));
			Graphics_textRect (
			        my graphics (), t1, t2, 0.0, 1.0, interval->text.get ());
	...
	*/
	...
	for (integer ipoint = 1; ipoint <= numberOfPoints; ipoint++) {
		...
		if (t >= my startWindow () && t <= my endWindow ()) {
			const bool pointIsSelected =
			        (itier == my selectedTier && t == my startSelection ());
			Graphics_setColour (my graphics (),
			        DataGui_defaultForegroundColour (me, pointIsSelected));
			Graphics_setLineWidth (
			        my graphics (), platformUsesAntiAliasing ? 6.0 : 5.0);
			...
			*/
			if (my startSelection () == t || my endSelection () == t) {
				Graphics_setLineWidth (my graphics (), 1.0);
				Graphics_setColour (
				        my graphics (), DataGuiColour_AREA_BACKGROUND);
...
void structTextGridArea ::v_drawInside () {
	...
	Graphics_setWindow (
	        our graphics (), our startWindow (), our endWindow (), 0.0, 1.0);
	for (integer itier = 1; itier <= numberOfTiers; itier++) {
		const Function anyTier = our textGrid () -> tiers->at[itier];
		...
		Graphics_setColour (our graphics (), Melder_BLACK);
		if (itier != 1)
			Graphics_line (our graphics (), our startWindow (), 1.0,
			        our endWindow (), 1.0);
		...
		*/
		Graphics_setColour (our graphics (),
		        DataGui_defaultForegroundColour (this, tierIsSelected));
		Graphics_setFont (our graphics (), oldFont);
		Graphics_setFontSize (our graphics (), 14);
		Graphics_setTextAlignment (
		        our graphics (), Graphics_RIGHT, Graphics_HALF);
		Graphics_text (our graphics (), our startWindow (), 0.5,
		        tierIsSelected ? U"☞ " : U"", itier);
		integer fontSize = 9;
		if (numberOfTiers < 5) fontSize = 10;
		if (numberOfTiers <= 3) fontSize = 12;
		if (numberOfTiers >= 6) fontSize = 7;
		if (numberOfTiers > 5) fontSize = 8;
		Graphics_setFontSize (our graphics (),
		        our borrowedSoundArea && our borrowedSoundAnalysisArea
		                ? fontSize
		                : oldFontSize);
		if (anyTier->name && anyTier->name[0]) {
			Graphics_setTextAlignment (our graphics (), Graphics_LEFT,
			        our instancePref_showNumberOf () ==
			                        kTextGridArea_showNumberOf::NOTHING
			                ? Graphics_HALF
			                : Graphics_BOTTOM);
			Graphics_text (our graphics (), our endWindow (), 0.5,
			        anyTier->name.get ());
		}
		if (our instancePref_showNumberOf () !=
		        kTextGridArea_showNumberOf::NOTHING) {
			Graphics_setTextAlignment (
			        our graphics (), Graphics_LEFT, Graphics_TOP);
			if (our instancePref_showNumberOf () ==
			        kTextGridArea_showNumberOf::INTERVALS_OR_POINTS) {
				const integer count =
				        (isIntervalTier ? ((IntervalTier) anyTier)
				                                  ->intervals.size
				                        : ((TextTier) anyTier)->points.size);
				const integer position =
				        (itier == our selectedTier
				                        ? (isIntervalTier ? getSelectedInterval (
				                                                    this)
				                                          : getSelectedPoint (
				                                                    this))
				                        : 0);
				if (position)
					Graphics_text (our graphics (), our endWindow (), 0.5, U"(",
					        position, U"/", count, U")");
				else
					Graphics_text (our graphics (), our endWindow (), 0.5, U"(",
					        count, U")");
			} else {
				Melder_assert (our instancePref_showNumberOf () ==
				               kTextGridArea_showNumberOf::
				                       NONEMPTY_INTERVALS_OR_POINTS);
				integer count = 0;
				if (isIntervalTier) {
					...
					const integer numberOfPoints = tier->points.size;
					for (integer ipoint = 1; ipoint <= numberOfPoints;
					        ipoint++) {
						const TextPoint point = tier->points.at[ipoint];
						if (point->mark && point->mark[0] != U'\0') count++;
					}
				}
				Graphics_text (our graphics (), our endWindow (), 0.5, U"(##",
				        count, U"#)");
	...
	if (isdefined (our draggingTime) &&
	        hasBeenDraggedBeyondVicinityRadiusAtLeastOnce) {
		...
		for (integer itier = 1; itier <= numberOfTiers; itier++) {
			if (our draggingTiers[itier]) {
				...
				Graphics_line (our graphics (), our draggingTime, ymin,
				        our draggingTime, ymax);
			}
		}
		our functionEditor () -> viewDataAsWorldByFraction ();
		Graphics_setLineWidth (our graphics (), 1.0);
		Graphics_line (
		        our graphics (), our draggingTime, 0.0, our draggingTime, 1.0);
		Graphics_text (our graphics (), our draggingTime, 1.0,
		        Melder_fixed (our draggingTime, 6));
...
static integer localY_fraction_toTier (
        TextGridArea me, double localY_fraction) {
	const integer numberOfTiers = my textGrid () -> tiers->size;
	integer tierNumber = numberOfTiers - Melder_ifloor (localY_fraction *
	                                                    (double) numberOfTiers);
	Melder_clip (1_integer, &tierNumber, numberOfTiers);
	return tierNumber;
}

bool structTextGridArea ::v_mouse (GuiDrawingArea_MouseEvent event,
	...
	constexpr double draggingVicinityRadius_mm =
	        clickingVicinityRadius_mm +
	        0.2;   // must be greater than `clickingVicinityRadius_mm`
	...
	if (event->isClick ()) {
		if (isdefined (our anchorTime))   // sanity check for the fixed order
		                                  // click-drag-drop
			return false;
		...
		*/
		...
		double startInterval, endInterval;
		timeToInterval (
		        this, x_world, our selectedTier, &startInterval, &endInterval);

		if (our borrowedSoundAnalysisArea &&
		        our instancePref_allTierNotes_show ())
			our borrowedSoundAnalysisArea->tierNotesData =
			        our instancePref_allTierNotes_show ()
			                ? getSelectedTierData (this)
			                : std::vector<std::vector<double>> {};

		if (event->isLeftBottomFunctionKeyPressed ()) {
			our setSelection (x_world - startInterval < endInterval - x_world
			                          ? startInterval
			                          : endInterval,
			        our endSelection ());   // to nearest boundary
			return FunctionEditor_UPDATE_NEEDED;
		}
		if (event->isRightBottomFunctionKeyPressed ()) {
			our setSelection (our startSelection (),
			        x_world - startInterval < endInterval - x_world
			                ? startInterval
			                : endInterval);
			return FunctionEditor_UPDATE_NEEDED;
		}
		...
		AnyTextGridTier_identifyClass (
		        our textGrid ()->tiers->at[our selectedTier],
		        &selectedIntervalTier, &selectedTextTier);
		...
		*/
		if (selectedIntervalTier) {
			const integer clickedIntervalNumber =
			        IntervalTier_timeToIndex (selectedIntervalTier, x_world);
			const bool theyClickedOutsidetheTimeDomainOfTheIntervals =
			        (clickedIntervalNumber == 0);
			if (theyClickedOutsidetheTimeDomainOfTheIntervals)
				return FunctionEditor_UPDATE_NEEDED;
			const TextInterval interval =
			        selectedIntervalTier->intervals.at[clickedIntervalNumber];
			...
			const integer clickedPointNumber = AnyTier_timeToNearestIndex (
			        selectedTextTier->asAnyTier (), x_world);
			if (clickedPointNumber != 0) {
				const TextPoint point =
				        selectedTextTier->points.at[clickedPointNumber];
				our anchorTime = point->number;
			}
		}
		Melder_assert (!(selectedIntervalTier && our clickedLeftBoundary == 0));

		const bool nearBoundaryOrPoint =
		        (isdefined (our anchorTime) &&
		                fabs (Graphics_dxWCtoMM (our graphics (),
		                        x_world - our anchorTime)) < 1.5);
		Graphics_setWindow (our graphics (), our startWindow (),
		        our endWindow (), 0.0, 1.0);
		const double distanceToCursorCircle =
		        (our startSelection () != our endSelection ()
		                        ? undefined
		                        : Graphics_distanceWCtoMM (our graphics (),
		                                  x_world, localY_fraction,
		                                  our startSelection (),
		                                  (numberOfTiers + 1 -
		                                          our selectedTier) /
		                                                  double (numberOfTiers) -
		...
		if (nearBoundaryOrPoint) {
			...
			*/
			...
			if (selectedIntervalTier) {
				const bool isLeftEdgeOfFirstInterval =
				        (our clickedLeftBoundary <= 1);
				const bool isRightEdgeOfLastInterval =
				        (our clickedLeftBoundary >
				                selectedIntervalTier->intervals.size);
				boundaryOrPointIsMovable = !isLeftEdgeOfFirstInterval &&
				                           !isRightEdgeOfLastInterval;
			}
			/*
			    If the user clicked on an unselected boundary or point, we
			   extend or shrink the selection to it.
			*/
			if (event->shiftKeyPressed) {
				if (our anchorTime >
				        0.5 * (our startSelection () + our endSelection ()))
			...
			/*
			    Determine the set of selected boundaries and points, and the
			   dragging range.
			*/
			...
			for (int itier = 1; itier <= numberOfTiers; itier++) {
				/*
				    If the user has pressed the shift key, let her drag all the
				   boundaries and points at this time. Otherwise, let her only
				   drag the boundary or point on the clicked tier.
				*/
				if (itier == mouseTier ||
				        our functionEditor ()->clickWasModifiedByShiftKey ==
				                our instancePref_shiftDragMultiple ()) {
					IntervalTier intervalTier;
					TextTier textTier;
					AnyTextGridTier_identifyClass (
					        our textGrid ()->tiers->at[itier], &intervalTier,
					        &textTier);
					if (intervalTier) {
						const integer ibound = IntervalTier_hasBoundary (
						        intervalTier, our anchorTime);
						if (ibound) {
							TextInterval leftInterval =
							        intervalTier->intervals.at[ibound - 1];
							TextInterval rightInterval =
							        intervalTier->intervals.at[ibound];
							our draggingTiers[itier] = true;
							/*
							    Prevent the user from dragging the boundary past
							   its left or right neighbours on the same tier.
							*/
							Melder_clipLeft (leftInterval->xmin,
							        &our leftDraggingBoundary);
							Melder_clipRight (&our rightDraggingBoundary,
							        rightInterval->xmax);
						}
					} else {
						Melder_assert (isdefined (our anchorTime));
						if (AnyTier_hasPoint (
						            textTier->asAnyTier (), our anchorTime)) {
							/*
							    Other than with boundaries on interval tiers,
							    points on text tiers can be dragged past their
							   neighbours.
							*/
							our draggingTiers[itier] = true;
						}
			...
			/*
			    ...
			    Insert boundary or point. There is no danger that we insert on
			   top of an existing boundary or point, because we are not
			   'nearBoundaryOrPoint'.
			*/
			Melder_assert (isdefined (
			        our startSelection ()));   // precondition of v_updateText()
			if (our selectedTier != oldSelectedTier)
				our v_updateText ();   // this puts the text of the newly
				                       // clicked tier into the text area
			insertBoundaryOrPoint (this, mouseTier, our startSelection (),
			        our startSelection (), false);
		...
		if (our isMoveWhileButtonDown && our editable () &&
		        isdefined (our anchorTime) && our draggingTiers.size > 0) {
			our draggingTime = x_world;
			if (!our hasBeenDraggedBeyondVicinityRadiusAtLeastOnce) {
				const double distanceToAnchor_mm = fabs (Graphics_dxWCtoMM (
				        our graphics (), x_world - our anchorTime));
				if (distanceToAnchor_mm > draggingVicinityRadius_mm)
					our hasBeenDraggedBeyondVicinityRadiusAtLeastOnce = true;
			}
		}
		// return FunctionEditor_defaultMouseInWideDataView (our
		// functionEditor(), event, x_world);
	} else if (event->isDrop ()) {
		if (isundef (our anchorTime) ||
		        our draggingTiers.size ==
		                0) {   // TODO: figure out a circumstance under which
			                   // anchorTime could be undefined
			our draggingTime = undefined;
			our hasBeenDraggedBeyondVicinityRadiusAtLeastOnce = false;
			our anchorTime = undefined;
			our clickedLeftBoundary = 0;
		...
		/*
		    If the user shift-clicked, we extend the selection (this already
		   happened during click()).
		*/
		if (event->shiftKeyPressed &&
		        !our hasBeenDraggedBeyondVicinityRadiusAtLeastOnce) {
			our draggingTime = undefined;
			our hasBeenDraggedBeyondVicinityRadiusAtLeastOnce = false;
			our anchorTime = undefined;
			our clickedLeftBoundary = 0;
		...
		*/
		const integer itierDrop =
		        localY_fraction_toTier (this, localY_fraction);
		bool droppedOnABoundaryOrPointInsideAnUnselectedTier = false;
		if (x_world > 0.0 &&
		        !our draggingTiers[itierDrop]) {   // dropped inside an
			                                       // unselected tier?
			const Function anyTierDrop =
			        our textGrid () -> tiers->at[itierDrop];
			if (anyTierDrop->classInfo == classIntervalTier) {
				const IntervalTier tierDrop = (IntervalTier) anyTierDrop;
				for (integer ibound = 1; ibound < tierDrop->intervals.size;
				        ibound++) {
					const TextInterval left = tierDrop->intervals.at[ibound];
					const double mouseDistanceToBoundary =
					        fabs (Graphics_dxWCtoMM (
					                our graphics (), x_world - left->xmax));
				...
				for (integer ipoint = 1; ipoint <= tierDrop->points.size;
				        ipoint++) {
					const TextPoint point = tierDrop->points.at[ipoint];
					const double mouseDistanceToPoint_mm =
					        fabs (Graphics_dxWCtoMM (
					                our graphics (), x_world - point->number));
		...
		*/
		if (our startSelection () == our endSelection () &&
			...
			const double mouseDistanceToCursor = fabs (Graphics_dxWCtoMM (
			        our graphics (), x_world - our startSelection ()));
			if (mouseDistanceToCursor < droppingVicinityRadius_mm)
				x_world = our startSelection ();
		}
		/*
		    If the user wiggled near the anchor, we snap to the anchor and bail
		   out (a boundary has been selected, but nothing has been dragged).
		*/
		if (!our hasBeenDraggedBeyondVicinityRadiusAtLeastOnce &&
		...
		*/
		if (x_world <= our leftDraggingBoundary ||
		        x_world >= our rightDraggingBoundary) {
			Melder_beep ();
			our draggingTime = undefined;
			our hasBeenDraggedBeyondVicinityRadiusAtLeastOnce = false;
			our anchorTime = undefined;
		...
		for (integer itier = 1; itier <= numberOfTiers; itier++) {
			if (our draggingTiers[itier]) {
				...
				AnyTextGridTier_identifyClass (
				        ...
				        &textTier);
				if (intervalTier) {
					const integer numberOfIntervals =
					        intervalTier->intervals.size;
					for (integer ibound = 2; ibound <= numberOfIntervals;
					        ibound++) {
						TextInterval left = intervalTier->intervals
						                            .at[ibound - 1],
						             right = intervalTier->intervals.at[ibound];
						if (left->xmax ==
						        our anchorTime) {   // boundary dragged?
							left->xmax = right->xmin =
							        x_world;   // move boundary to drop site
							break;
						}
					}
				} else {
					Melder_assert (isdefined (our anchorTime));
					const integer iDraggedPoint = AnyTier_hasPoint (
					        textTier->asAnyTier (), our anchorTime);
					if (iDraggedPoint) {
						Melder_assert (isdefined (x_world));
						const integer dropSiteHasPoint = AnyTier_hasPoint (
						        textTier->asAnyTier (), x_world);
						if (dropSiteHasPoint != 0) {
							Melder_warning (
							        U"Cannot drop point on an existing point.");
						} else {
							const TextPoint point =
							        textTier->points.at[iDraggedPoint];
							/*
							    Move point to drop site. May have passed another
							   point.
							*/
							autoTextPoint newPoint = Data_copy (point);
							newPoint->number =
							        x_world;   // move point to drop site
		...
		*/
		...
		if (selectedIntervalTier) {
			...
			const bool nearBoundary =
			        (isdefined (dblclk_anchorTime) &&
			                fabs (Graphics_dxWCtoMM (our graphics (),
			                        x_world - dblclk_anchorTime)) < 1.5);

			if (nearBoundary) {
				FunctionArea_save (this, U"Remove boundary");
				IntervalTier_removeLeftBoundary (
				        selectedIntervalTier, dblclk_clickedLeftBoundary);
				FunctionArea_broadcastDataChanged (this);
			}
...
void structTextGridArea ::v_handleMouseDBLCLK (double x_world) {
	try {
		...
		FunctionArea_broadcastDataChanged (this);
	} catch (MelderError) { Melder_throw (U"Boundary or point not inserted."); }
}
...
void structTextGridArea ::v_createMenuItems_save (EditorMenu menu) {
	FunctionAreaMenu_addCommand (
	        menu, U"- Save TextGrid to disk:", 0, nullptr, this);
	FunctionAreaMenu_addCommand (menu,
	        U"Save whole TextGrid as text file... || Save TextGrid as text "
	        U"file... || Write TextGrid to text file...",
...
static void menu_cb_ConvertToBackslashTrigraphs (TextGridArea me, EDITOR_ARGS) {
	...
	Melder_assert (
	        isdefined (my startSelection ()));   // precondition of
...
static void findInTier (TextGridArea me) {
	...
	if (anyTier->classInfo == classIntervalTier) {
		const IntervalTier tier = (IntervalTier) anyTier;
		integer iinterval =
		        IntervalTier_timeToIndex (tier, my startSelection ()) + 1;
				...
				if (position) {
					...
					Melder_assert (isdefined (
					        my startSelection ()));   // precondition of
					                                  // FunctionEditor_scrollToView()
					FunctionEditor_scrollToView (
					        my functionEditor (), my startSelection ());
					if (my editable ())
						GuiText_setSelection (my functionEditor ()->textArea,
						        position - text,
						        position - text +
						                Melder_length (my findString.get ()));
					return;
				}
			}
			iinterval++;
		}
		if (iinterval > tier->intervals.size) Melder_beep ();
	} else {
		TextTier tier = (TextTier) anyTier;
		integer ipoint = AnyTier_timeToLowIndex (
		                         tier->asAnyTier (), my startSelection ()) +
		                 1;
		while (ipoint <= tier->points.size) {
			const TextPoint point = tier->points.at[ipoint];
			conststring32 text = point->mark.get ();
			if (text) {
				const char32 *const position =
				        str32str (text, my findString.get ());
				if (position) {
					...
					Melder_assert (isdefined (
					        my startSelection ()));   // precondition of
					                                  // FunctionEditor_scrollToView()
					FunctionEditor_scrollToView (
					        my functionEditor (), point->number);
					if (my editable ())
						GuiText_setSelection (my functionEditor ()->textArea,
						        position - text,
						        position - text +
		...
		if (ipoint > tier->points.size) Melder_beep ();
	}
}
static void do_find (TextGridArea me) {
	if (my findString) {
		if (my editable ()) {
			integer left, right;
			autostring32 label = GuiText_getStringAndSelectionPosition (
			        my functionEditor ()->textArea, &left, &right);
			const char32 *const position = str32str (
			        &label[right], my findString.get ());   // CRLF BUG?
			if (position)
				GuiText_setSelection (my functionEditor ()->textArea,
				        position - label.get (),
				        position - label.get () +
...
static void menu_cb_FindAgain (TextGridArea me, EDITOR_ARGS) { do_find (me); }
void structTextGridArea ::v_createMenuItems_edit (EditorMenu menu) {
#ifndef macintosh
	if (our editable ()) {
		FunctionAreaMenu_addCommand (
		        menu, U"-- cut copy paste --", 0, nullptr, this);
		FunctionAreaMenu_addCommand (menu, U"Cut text", 'X', menu_cb_Cut, this);
		FunctionAreaMenu_addCommand (
		        menu, U"Cut", GuiMenu_HIDDEN, menu_cb_Cut, this);
		FunctionAreaMenu_addCommand (
		        menu, U"Copy text", 'C', menu_cb_Copy, this);
		FunctionAreaMenu_addCommand (
		        menu, U"Copy", GuiMenu_HIDDEN, menu_cb_Copy, this);
		FunctionAreaMenu_addCommand (
		        menu, U"Paste text", 'V', menu_cb_Paste, this);
		FunctionAreaMenu_addCommand (
		        menu, U"Paste", GuiMenu_HIDDEN, menu_cb_Paste, this);
		FunctionAreaMenu_addCommand (
		        menu, U"Erase text", 0, menu_cb_Erase, this);
		FunctionAreaMenu_addCommand (
		        menu, U"Erase", GuiMenu_HIDDEN, menu_cb_Erase, this);
	}
#endif
	FunctionAreaMenu_addCommand (menu, U"-- search --", 0, nullptr, this);
	FunctionAreaMenu_addCommand (menu, U"Find...", 'F', menu_cb_Find, this);
	FunctionAreaMenu_addCommand (
	        menu, U"Find again", 'G', menu_cb_FindAgain, this);
}

#pragma mark - TextGridArea Query

static void QUERY_DATA_FOR_REAL__GetStartingPointOfInterval (
        TextGridArea me, EDITOR_ARGS) {
	QUERY_DATA_FOR_REAL
	...
	const IntervalTier tier = (IntervalTier) anyTier;
	const integer iinterval =
	        IntervalTier_timeToIndex (tier, my startSelection ());
	const double result = (iinterval < 1 || iinterval > tier->intervals.size
	                               ? undefined
	                               : tier->intervals.at[iinterval]->xmin);
	QUERY_DATA_FOR_REAL_END (U" seconds")
}

static void QUERY_DATA_FOR_REAL__GetEndPointOfInterval (
        TextGridArea me, EDITOR_ARGS) {
	QUERY_DATA_FOR_REAL
...
static void QUERY_DATA_FOR_STRING__GetLabelOfInterval (
        TextGridArea me, EDITOR_ARGS) {
	...
	const integer iinterval =
	        IntervalTier_timeToIndex (tier, my startSelection ());
	const conststring32 result =
	        (iinterval < 1 || iinterval > tier->intervals.size
	                        ? U""
...
static void gui_text_cb_changed (TextGridArea me, GuiTextEvent /* event */) {
	...
	if (my suppressRedraw)
		return;   // prevent infinite loop if 'draw' method or
		          // Editor_broadcastChange calls GuiText_setString
	if (my selectedTier) {
		autostring32 text = GuiText_getString (my functionEditor ()->textArea);
		IntervalTier intervalTier;
		TextTier textTier;
		AnyTextGridTier_identifyClass (
		        my textGrid ()->tiers->at[my selectedTier], &intervalTier,
		        &textTier);
		if (intervalTier) {
			const integer selectedInterval = getSelectedInterval (me);
			if (selectedInterval) {
				TextInterval interval =
				        intervalTier->intervals.at[selectedInterval];
				TextInterval_setText (interval, text.get ());
				if (my borrowedSoundAnalysisArea &&
				        my instancePref_allTierNotes_show ())
					my borrowedSoundAnalysisArea->tierNotesData =
					        my instancePref_allTierNotes_show ()
					                ? getSelectedTierData (me)
					                : std::vector<std::vector<double>> {};
...
static void soundAnalysisArea_cb_clickToChangePitch (
        TextGridArea me, double newPitch) {
	trace (my suppressRedraw);
	if (my suppressRedraw)
		return;   // prevent infinite loop if 'draw' method or
		          // Editor_broadcastChange calls GuiText_setString
	if (my selectedTier) {
		...
		if (intervalTier) {
			...
			if (selectedInterval) {
				TextInterval interval =
				        intervalTier->intervals.at[selectedInterval];
				TextInterval_setText (interval, Melder_fixed (newPitch, 2));
				my borrowedSoundAnalysisArea->tierNotesData =
				        my instancePref_allTierNotes_show ()
				                ? getSelectedTierData (me)
				                : std::vector<std::vector<double>> {};
...
static void menu_cb_TextGridPreferences (TextGridArea me, EDITOR_ARGS) {
	...
	OPTIONMENU_ENUM (kGraphics_horizontalAlignment, textAlignmentInIntervals,
	        U"Text alignment in intervals",
	        kGraphics_horizontalAlignment::DEFAULT)
	OPTIONMENU (useTextStyles, U"The symbols %#_^ in labels",
	        my default_useTextStyles () + 1)
	OPTION (U"are shown as typed")
	OPTION (U"mean italic/bold/sub/super")
	OPTIONMENU_VARIABLE (shiftDragMultiple)
	if (my editable ()) {
		OPTIONMENU_FIELD (shiftDragMultiple, U"With the shift key, you drag",
		        my default_shiftDragMultiple () + 1)
	...
	SET_ENUM (textAlignmentInIntervals, kGraphics_horizontalAlignment,
	        my instancePref_alignment ())
	if (my editable ())
		SET_OPTION (shiftDragMultiple, my instancePref_shiftDragMultiple () + 1)
	SET_ENUM (showNumberOf, kTextGridArea_showNumberOf,
	        my instancePref_showNumberOf ())
	SET_ENUM (paintIntervalsGreenWhoseLabel, kMelder_string,
	        my instancePref_greenMethod ())
...
static void do_selectAdjacentTier (TextGridArea me, bool previous) {
	...
	if (n >= 2) {
		my selectedTier =
		        (previous ? my selectedTier > 1 ? my selectedTier - 1 : n
		                : my selectedTier < n ? my selectedTier + 1
		                                      : 1);
		double startInterval, endInterval;
		timeToInterval (me, my startSelection (), my selectedTier,
...
static void do_selectAdjacentInterval (
        TextGridArea me, bool previous, bool shift) {
	IntervalTier intervalTier;
	TextTier textTier;
	if (my selectedTier < 1 || my selectedTier > my textGrid ()->tiers->size)
		return;
	AnyTextGridTier_identifyClass (my textGrid ()->tiers->at[my selectedTier],
	        &intervalTier, &textTier);
	if (intervalTier) {
		const integer n = intervalTier->intervals.size;
		if (n >= 2) {
			integer iinterval = IntervalTier_timeToIndex (
			        intervalTier, my startSelection ());
			if (shift) {
				const integer binterval = IntervalTier_timeToIndex (
				        intervalTier, my startSelection ());
				integer einterval = IntervalTier_timeToIndex (
				        intervalTier, my endSelection ());
				if (my endSelection () == intervalTier->xmax) einterval++;
				if (binterval < iinterval && einterval > iinterval + 1) {
					const TextInterval interval =
					        intervalTier->intervals.at[iinterval];
					my setSelection (interval->xmin, interval->xmax);
				} else if (previous) {
					if (einterval > iinterval + 1) {
						if (einterval <= n + 1) {
							const TextInterval interval =
							        intervalTier->intervals.at[einterval - 1];
							my setSelection (
							        my startSelection (), interval->xmin);
						}
					} else if (binterval > 1) {
						const TextInterval interval =
						        intervalTier->intervals.at[binterval - 1];
					...
					if (binterval < iinterval) {
						if (binterval > 0) {
							const TextInterval interval =
							        intervalTier->intervals.at[binterval];
							my setSelection (
							        interval->xmax, my endSelection ());
						}
					} else if (einterval <= n) {
						const TextInterval interval =
						        intervalTier->intervals.at[einterval];
						my setSelection (my startSelection (), interval->xmax);
					}
				}
			} else {
				iinterval = (previous        ? iinterval > 1 ? iinterval - 1 : n
				             : iinterval < n ? iinterval + 1
				                             : 1);
				const TextInterval interval =
				        intervalTier->intervals.at[iinterval];
				my setSelection (interval->xmin, interval->xmax);
			...
			Melder_assert (isdefined (
			        my startSelection ()));   // precondition of
			                                  // FunctionEditor_scrollToView()
			FunctionEditor_scrollToView (my functionEditor (),
			        iinterval == n   ? my startSelection ()
			        : iinterval == 1 ? my endSelection ()
		...
		const integer n = textTier->points.size;
		if (n >= 2) {
			integer ipoint = AnyTier_timeToHighIndex (
			        textTier->asAnyTier (), my startSelection ());
			ipoint = (previous     ? ipoint > 1 ? ipoint - 1 : n
			          : ipoint < n ? ipoint + 1
			                       : 1);
...
static void menu_cb_ExtendSelectPreviousInterval (
        TextGridArea me, EDITOR_ARGS) {
...
static void menu_cb_DrawVisibleTextGrid (TextGridArea me, EDITOR_ARGS) {
	...
	TextGrid_Sound_draw (my textGrid (), nullptr, my pictureGraphics (),
	        ...
	        my instancePref_useTextStyles (), garnish);
...
static void menu_cb_DrawVisibleSoundAndTextGrid (TextGridArea me, EDITOR_ARGS) {
	...
	DataGui_openPraatPicture (me);
	{   // scope
		autoSound sound = (my borrowedSoundArea->longSound ()
		                           ? LongSound_extractPart (
		                                     my borrowedSoundArea->longSound (),
		                                     my startWindow (), my endWindow (),
		                                     true)
		                           : Sound_extractPart (
		                                     my borrowedSoundArea->sound (),
		                                     my startWindow (), my endWindow (),
		                                     kSound_windowShape::RECTANGULAR,
		                                     1.0, true));
		TextGrid_Sound_draw (my textGrid (), sound.get (),
		        my pictureGraphics (), my startWindow (), my endWindow (), true,
		        my instancePref_useTextStyles (), garnish);
	}
	FunctionArea_garnishPicture (me);
	DataGui_closePraatPicture (me);
...
static void menu_cb_DrawTextGridAndPitch (TextGridArea me, EDITOR_ARGS) {
	...
	my v_form_pictureWindow (cmd);
	LABEL (U"TextGrid:")
	BOOLEAN (showBoundariesAndPoints, U"Show boundaries and points",
	        my default_picture_showBoundaries ());
	LABEL (U"Pitch:")
	...
	my v_ok_pictureWindow (cmd);
	SET_BOOLEAN (
	        showBoundariesAndPoints, my classPref_picture_showBoundaries ())
	...
	EDITOR_DO
	my v_do_pictureWindow (cmd);
	my setClassPref_picture_showBoundaries (
	        showBoundariesAndPoints);   // set prefs even if analyses are
	                                    // missing (it would be annoying not to)
	...
	const double pitchFloor_hidden = Function_convertStandardToSpecialUnit (
	        my borrowedSoundAnalysisArea->d_pitch.get (),
	        my borrowedSoundAnalysisArea->instancePref_pitch_floor (),
	        Pitch_LEVEL_FREQUENCY,
	        (int) my borrowedSoundAnalysisArea->instancePref_pitch_unit ());
	const double pitchCeiling_hidden = Function_convertStandardToSpecialUnit (
	        my borrowedSoundAnalysisArea->d_pitch.get (),
	        my borrowedSoundAnalysisArea->instancePref_pitch_ceiling (),
	        Pitch_LEVEL_FREQUENCY,
	        (int) my borrowedSoundAnalysisArea->instancePref_pitch_unit ());
	const double pitchFloor_overt = Function_convertToNonlogarithmic (
	        my borrowedSoundAnalysisArea->d_pitch.get (), pitchFloor_hidden,
	        Pitch_LEVEL_FREQUENCY,
	        (int) my borrowedSoundAnalysisArea->instancePref_pitch_unit ());
	const double pitchCeiling_overt = Function_convertToNonlogarithmic (
	        my borrowedSoundAnalysisArea->d_pitch.get (), pitchCeiling_hidden,
	        Pitch_LEVEL_FREQUENCY,
	        (int) my borrowedSoundAnalysisArea->instancePref_pitch_unit ());
	const double pitchViewFrom_overt =
	        (my borrowedSoundAnalysisArea->instancePref_pitch_viewFrom () <
	                                my borrowedSoundAnalysisArea
	                                        ->instancePref_pitch_viewTo ()
	                        ? my borrowedSoundAnalysisArea
	                                  ->instancePref_pitch_viewFrom ()
	                        : pitchFloor_overt);
	const double pitchViewTo_overt =
	        (my borrowedSoundAnalysisArea->instancePref_pitch_viewFrom () <
	                        ...
	                        ? my borrowedSoundAnalysisArea
	                                  ->instancePref_pitch_viewTo ()
	                        : pitchCeiling_overt);
	TextGrid_Pitch_drawSeparately (my textGrid (),
	        my borrowedSoundAnalysisArea->d_pitch.get (), my pictureGraphics (),
	        my startWindow (), my endWindow (), pitchViewFrom_overt,
	        pitchViewTo_overt, showBoundariesAndPoints,
	        my instancePref_useTextStyles (), garnish, speckle,
	        my borrowedSoundAnalysisArea->instancePref_pitch_unit ());
	FunctionArea_garnishPicture (me);
	DataGui_closePraatPicture (me);
	EDITOR_END
}
...
static void CONVERT_DATA_TO_ONE__ExtractSelectedTextGrid_preserveTimes (
        TextGridArea me, EDITOR_ARGS) {
	CONVERT_DATA_TO_ONE
	if (my endSelection () <= my startSelection ())
		Melder_throw (U"No selection.");
	autoTextGrid result = TextGrid_extractPart (
	        my textGrid (), my startSelection (), my endSelection (), true);
	CONVERT_DATA_TO_ONE_END (U"untitled")
}
static void CONVERT_DATA_TO_ONE__ExtractSelectedTextGrid_timeFromZero (
        TextGridArea me, EDITOR_ARGS) {
	CONVERT_DATA_TO_ONE
	if (my endSelection () <= my startSelection ())
		Melder_throw (U"No selection.");
	autoTextGrid result = TextGrid_extractPart (
	        my textGrid (), my startSelection (), my endSelection (), false);
	CONVERT_DATA_TO_ONE_END (U"untitled")
}
...
static void menu_cb_AlignInterval (TextGridArea me, EDITOR_ARGS) {
	...
	const AnyTier tier =
	        static_cast<AnyTier> (my textGrid ()->tiers->at[my selectedTier]);
	if (tier->classInfo != classIntervalTier)
		Melder_throw (U"Alignment works only for interval tiers, whereas tier ",
		        my selectedTier,
		        U" is a point tier.\nSelect an interval tier instead.");
	const integer intervalNumber = getSelectedInterval (me);
	if (!intervalNumber) Melder_throw (U"Select an interval first");
	if (!my instancePref_align_includeWords () &&
	        !my instancePref_align_includePhonemes ())
		Melder_throw (
		        U"Nothing to be done.\nPlease switch on \"Include words\" "
		        U"and/or \"Include phonemes\" in the \"Alignment settings\".");
	{   // scope
		const autoMelderProgressOff noprogress;
		FunctionArea_save (me, U"Align interval");
		TextGrid_anySound_alignInterval (my textGrid (),
		        my borrowedSoundArea->soundOrLongSound (), my selectedTier,
		        intervalNumber, my instancePref_align_language (),
		        my instancePref_align_includeWords (),
		        my instancePref_align_includePhonemes ());
	}
	FunctionArea_broadcastDataChanged (me);
}
static void menu_cb_AlignmentSettings (TextGridArea me, EDITOR_ARGS) {
	...
	OPTIONMENU (language, U"Language",
	        (int) Strings_findString (espeakdata_languages_names.get (),
	                U"English (Great Britain)"))
	...
	BOOLEAN (includePhonemes, U"Include phonemes",
	        my default_align_includePhonemes ())
	BOOLEAN (
	        allowSilences, U"Allow silences", my default_align_allowSilences ())
	EDITOR_OK
	int prefVoice = (int) Strings_findString (espeakdata_languages_names.get (),
	        my instancePref_align_language ());
	if (prefVoice == 0)
		prefVoice = (int) Strings_findString (
		        espeakdata_languages_names.get (), U"English (Great Britain)");
	...
	EDITOR_DO
	my setInstancePref_align_language (
	        espeakdata_languages_names->strings[language].get ());
...
static void do_insertIntervalOnTier (TextGridArea me, int itier) {
	try {
		insertBoundaryOrPoint (me, itier,
		        my functionEditor ()->duringPlay
		                ? my functionEditor ()->playCursor
		                : my startSelection (),
		        my functionEditor ()->duringPlay
		                ? my functionEditor ()->playCursor
		                : my endSelection (),
		...
		my selectedTier = itier;
		FunctionArea_broadcastDataChanged (me);
	} catch (MelderError) { Melder_throw (U"Interval not inserted."); }
}
static void menu_cb_InsertIntervalOnTier1 (TextGridArea me, EDITOR_ARGS) {
	do_insertIntervalOnTier (me, 1);
}
static void menu_cb_InsertIntervalOnTier2 (TextGridArea me, EDITOR_ARGS) {
	do_insertIntervalOnTier (me, 2);
}
static void menu_cb_InsertIntervalOnTier3 (TextGridArea me, EDITOR_ARGS) {
	do_insertIntervalOnTier (me, 3);
}
static void menu_cb_InsertIntervalOnTier4 (TextGridArea me, EDITOR_ARGS) {
	do_insertIntervalOnTier (me, 4);
}
static void menu_cb_InsertIntervalOnTier5 (TextGridArea me, EDITOR_ARGS) {
	do_insertIntervalOnTier (me, 5);
}
static void menu_cb_InsertIntervalOnTier6 (TextGridArea me, EDITOR_ARGS) {
	do_insertIntervalOnTier (me, 6);
}
static void menu_cb_InsertIntervalOnTier7 (TextGridArea me, EDITOR_ARGS) {
	do_insertIntervalOnTier (me, 7);
}
static void menu_cb_InsertIntervalOnTier8 (TextGridArea me, EDITOR_ARGS) {
	do_insertIntervalOnTier (me, 8);
}
...
static void do_movePointOrBoundary (TextGridArea me, int where) {
	...
	if (anyTier->classInfo == classIntervalTier) {
		const IntervalTier tier = (IntervalTier) anyTier;
		static const conststring32 boundarySaveText[3]{
		        U"Move boundary to zero crossing", U"Move boundary to B",
		        U"Move boundary to E"};
		const integer selectedLeftBoundary = getSelectedLeftBoundary (me);
		if (selectedLeftBoundary == 0)
		...
		const double position =
		        (where == 1          ? my startSelection ()
		                : where == 2 ? my endSelection ()
		                             : Sound_getNearestZeroCrossing (
		                                       my borrowedSoundArea->sound (),
		                                       left->xmax, 1));   // STEREO BUG
		if (isundef (position))
		...
		TextTier tier = (TextTier) anyTier;
		static const conststring32 pointSaveText[3]{
		        U"Move point to zero crossing", U"Move point to B",
		        U"Move point to E"};
		const integer selectedPoint = getSelectedPoint (me);
		if (selectedPoint == 0)
...
static void menu_cb_InsertOnTier1 (TextGridArea me, EDITOR_ARGS) {
	do_insertOnTier (me, 1);
}
static void menu_cb_InsertOnTier2 (TextGridArea me, EDITOR_ARGS) {
	do_insertOnTier (me, 2);
}
static void menu_cb_InsertOnTier3 (TextGridArea me, EDITOR_ARGS) {
	do_insertOnTier (me, 3);
}
static void menu_cb_InsertOnTier4 (TextGridArea me, EDITOR_ARGS) {
	do_insertOnTier (me, 4);
}
static void menu_cb_InsertOnTier5 (TextGridArea me, EDITOR_ARGS) {
	do_insertOnTier (me, 5);
}
static void menu_cb_InsertOnTier6 (TextGridArea me, EDITOR_ARGS) {
	do_insertOnTier (me, 6);
}
static void menu_cb_InsertOnTier7 (TextGridArea me, EDITOR_ARGS) {
	do_insertOnTier (me, 7);
}
static void menu_cb_InsertOnTier8 (TextGridArea me, EDITOR_ARGS) {
	do_insertOnTier (me, 8);
}
...
static void menu_cb_InsertOnAllTiers (TextGridArea me, EDITOR_ARGS) {
	...
	my selectedTier =
	        saveTier;   // only if everything went right; otherwise, the tier
	                    // where something went wrong will stand selected
}
...
static void menu_cb_AddIntervalTier (TextGridArea me, EDITOR_ARGS) {
	...
	EDITOR_OK
	SET_INTEGER_AS_STRING (position,
	        Melder_cat (my textGrid ()->tiers->size + 1, U" (= at bottom)"))
	SET_STRING (name, U"")
	EDITOR_DO {   // scope
		autoIntervalTier tier = IntervalTier_create (
		        my textGrid ()->xmin, my textGrid ()->xmax);
		Melder_clipRight (&position, my textGrid ()->tiers->size + 1);
		Thing_setName (tier.get (), name);

		FunctionArea_save (me, U"Add interval tier");
		my textGrid ()
		        -> tiers->addItemAtPosition_move (tier.move (), position);
	}
...
static void menu_cb_AddPointTier (TextGridArea me, EDITOR_ARGS) {
	...
	EDITOR_DO {   // scope
		autoTextTier tier =
		        TextTier_create (my textGrid ()->xmin, my textGrid ()->xmax);
		Melder_clipRight (&position, my textGrid ()->tiers->size + 1);
		Thing_setName (tier.get (), name);
...
static void menu_cb_DuplicateTier (TextGridArea me, EDITOR_ARGS) {
	...
	if (my selectedTier != 0) {
		...
		SET_STRING (
		        name, my textGrid ()->tiers->at[my selectedTier]->name.get ())
	}
	EDITOR_DO
	...
	{   // scope
		...
		my textGrid ()
		        -> tiers->addItemAtPosition_move (newTier.move (), position);
	}
...
static void checkSpellingInTier (TextGridArea me) {
	...
	if (anyTier->classInfo == classIntervalTier) {
		...
		while (iinterval <= tier->intervals.size) {
			...
			if (text) {
				integer position = 0;
				conststring32 notAllowed = SpellingChecker_nextNotAllowedWord (
				        my spellingChecker, text, &position);
				if (notAllowed) {
					my setSelection (interval->xmin, interval->xmax);
					Melder_assert (isdefined (
					        my startSelection ()));   // precondition of
...
static void menu_cb_CheckSpelling (TextGridArea me, EDITOR_ARGS) {
	if (my spellingChecker) {
		...
		conststring32 notAllowed = SpellingChecker_nextNotAllowedWord (
		        my spellingChecker, label.get (), &position);
		if (notAllowed)
			GuiText_setSelection (my functionEditor ()->textArea, position,
			        position + Melder_length (notAllowed));
		else
...
static void menu_cb_AddToUserDictionary (TextGridArea me, EDITOR_ARGS) {
	if (my spellingChecker) {
		const autostring32 word =
		        GuiText_getSelection (my functionEditor ()->textArea);
...
static void menu_cb_showAllBoundaryLines (TextGridArea me, EDITOR_ARGS) {
	VOID_EDITOR
	my setInstancePref_allBoundaryLines_show (
	        !my instancePref_allBoundaryLines_show ());   // toggle
	GuiMenuItem_check (my boundaryLinesToggle,
	        my instancePref_allBoundaryLines_show ());   // in case we're called
	                                                     // from a script
	FunctionEditor_redraw (my functionEditor ());
	VOID_EDITOR_END
}
...
static void menu_cb_showAllTierNotes (TextGridArea me, EDITOR_ARGS) {
	VOID_EDITOR
	my setInstancePref_allTierNotes_show (
	        !my instancePref_allTierNotes_show ());   // toggle
	GuiMenuItem_check (my tierNotesToggle,
	        my instancePref_allTierNotes_show ());   // in case we're called
	                                                 // from a script
	my borrowedSoundAnalysisArea->tierNotesData =
	        my instancePref_allTierNotes_show ()
	                ? getSelectedTierData (me)
...
static void menu_cb_tierNotesFrequencyResolution005 (
        TextGridArea me, EDITOR_ARGS) {
	VOID_EDITOR
	my setInstancePref_tierNotesFrequencyResolution (0.05);
	GuiMenuItem_check (my tierNotesResolution005Toggle, true);
	GuiMenuItem_check (my tierNotesResolution025Toggle, false);
	if (my borrowedSoundAnalysisArea)
		my borrowedSoundAnalysisArea->tierNotesFrequencyResolution = 0.05;
	FunctionEditor_redraw (my functionEditor ());
	VOID_EDITOR_END
}

static void menu_cb_tierNotesFrequencyResolution025 (
        TextGridArea me, EDITOR_ARGS) {
	VOID_EDITOR
	my setInstancePref_tierNotesFrequencyResolution (0.25);
	GuiMenuItem_check (my tierNotesResolution005Toggle, false);
	GuiMenuItem_check (my tierNotesResolution025Toggle, true);
	if (my borrowedSoundAnalysisArea)
		my borrowedSoundAnalysisArea->tierNotesFrequencyResolution = 0.25;
	FunctionEditor_redraw (my functionEditor ());
	VOID_EDITOR_END
}
...
static void menu_cb_viewProportion (TextGridArea me, EDITOR_ARGS) {
	...
	POSITIVE (soundAreaHeight, U"the height proportion of SoundArea (%)",
	        my default_soundAreaHeight ())
	POSITIVE (soundAnalysisAreaHeight,
	        U"the height proportion of SoundAnalysisArea (%)",
	        my default_soundAnalysisAreaHeight ())
	POSITIVE (textGridAreaHeight, U"the height proportion of TextGridArea (%)",
	        my default_textGridAreaHeight ())
	EDITOR_OK
	SET_REAL (soundAreaHeight, my instancePref_soundAreaHeight ())
	SET_REAL (
	        soundAnalysisAreaHeight, my instancePref_soundAnalysisAreaHeight ())
	SET_REAL (textGridAreaHeight, my instancePref_textGridAreaHeight ())
	EDITOR_DO
	Melder_require (textGridAreaHeight > 0 && soundAreaHeight > 0 &&
	                        soundAnalysisAreaHeight > 0,
...
void structTextGridArea ::v_createMenus () {
	EditorMenu textGridMenu =
	        Editor_addMenu (our functionEditor (), U"TextGrid", 0);
	FunctionAreaMenu_addCommand (textGridMenu, U"TextGrid preferences...", 0,
	        menu_cb_TextGridPreferences, this);
	if (editable ()) {
		FunctionAreaMenu_addCommand (
		        textGridMenu, U"-- encoding --", 0, nullptr, this);
		FunctionAreaMenu_addCommand (textGridMenu,
		        U"Convert entire TextGrid to backslash trigraphs", 0,
		        menu_cb_ConvertToBackslashTrigraphs, this);
		FunctionAreaMenu_addCommand (textGridMenu,
		        U"Convert entire TextGrid to Unicode", 0,
		        menu_cb_ConvertToUnicode, this);
	}

	FunctionAreaMenu_addCommand (
	        textGridMenu, U"- Select by TextGrid:", 0, nullptr, this);
	FunctionAreaMenu_addCommand (textGridMenu, U"Select previous tier",
	        GuiMenu_OPTION | GuiMenu_UP_ARROW | GuiMenu_DEPTH_1,
	        menu_cb_SelectPreviousTier, this);
	FunctionAreaMenu_addCommand (textGridMenu, U"Select next tier",
	        GuiMenu_OPTION | GuiMenu_DOWN_ARROW | GuiMenu_DEPTH_1,
	        menu_cb_SelectNextTier, this);
	FunctionAreaMenu_addCommand (textGridMenu, U"Select previous interval",
	        GuiMenu_OPTION | GuiMenu_LEFT_ARROW | GuiMenu_DEPTH_1,
	        menu_cb_SelectPreviousInterval, this);
	FunctionAreaMenu_addCommand (textGridMenu, U"Select next interval",
	        GuiMenu_OPTION | GuiMenu_RIGHT_ARROW | GuiMenu_DEPTH_1,
	        menu_cb_SelectNextInterval, this);
	FunctionAreaMenu_addCommand (textGridMenu, U"Extend-select left",
	        GuiMenu_SHIFT | GuiMenu_OPTION | GuiMenu_LEFT_ARROW |
	                GuiMenu_DEPTH_1,
	        menu_cb_ExtendSelectPreviousInterval, this);
	FunctionAreaMenu_addCommand (textGridMenu, U"Extend-select right",
	        GuiMenu_SHIFT | GuiMenu_OPTION | GuiMenu_RIGHT_ARROW |
	                GuiMenu_DEPTH_1,
	        menu_cb_ExtendSelectNextInterval, this);

	FunctionAreaMenu_addCommand (textGridMenu,
	        U"- Draw TextGrid to picture window:", 0, nullptr, this);
	...
	if (our borrowedSoundArea)
		FunctionAreaMenu_addCommand (textGridMenu,
		        U"Draw visible sound and TextGrid...", 1,
		        menu_cb_DrawVisibleSoundAndTextGrid, this);
	if (our borrowedSoundAnalysisArea)
		FunctionAreaMenu_addCommand (textGridMenu,
		        U"Draw visible pitch contour and TextGrid...", 1,
		        menu_cb_DrawTextGridAndPitch, this);

	FunctionAreaMenu_addCommand (textGridMenu,
	        U"- Extract TextGrid to objects window:", 0, nullptr, this);
	our extractSelectedTextGridPreserveTimesButton =
	        FunctionAreaMenu_addCommand (textGridMenu,
	                U"Extract selected TextGrid (preserve times)", 1,
	                CONVERT_DATA_TO_ONE__ExtractSelectedTextGrid_preserveTimes,
	                this);
	our extractSelectedTextGridTimeFromZeroButton =
	        FunctionAreaMenu_addCommand (textGridMenu,
	                U"Extract selected TextGrid (time from 0)", 1,
	                CONVERT_DATA_TO_ONE__ExtractSelectedTextGrid_timeFromZero,
	                this);

	EditorMenu intervalMenu =
	        Editor_addMenu (our functionEditor (), U"Interval", 0);
	if (our editable ()) {
		if (our borrowedSoundArea) {
			FunctionAreaMenu_addCommand (intervalMenu, U"Align interval", 'D',
			        menu_cb_AlignInterval, this);
			FunctionAreaMenu_addCommand (intervalMenu, U"Alignment settings...",
			        0, menu_cb_AlignmentSettings, this);
			FunctionAreaMenu_addCommand (
			        intervalMenu, U"-- after align --", 0, nullptr, this);
		}
		FunctionAreaMenu_addCommand (
		        intervalMenu, U"New interval:", 0, nullptr, this);
		FunctionAreaMenu_addCommand (intervalMenu, U"Add interval on tier 1",
		        GuiMenu_COMMAND | '1' | GuiMenu_DEPTH_1,
		        menu_cb_InsertIntervalOnTier1, this);
		FunctionAreaMenu_addCommand (intervalMenu, U"Add interval on tier 2",
		        GuiMenu_COMMAND | '2' | GuiMenu_DEPTH_1,
		        menu_cb_InsertIntervalOnTier2, this);
		FunctionAreaMenu_addCommand (intervalMenu, U"Add interval on tier 3",
		        GuiMenu_COMMAND | '3' | GuiMenu_DEPTH_1,
		        menu_cb_InsertIntervalOnTier3, this);
		FunctionAreaMenu_addCommand (intervalMenu, U"Add interval on tier 4",
		        GuiMenu_COMMAND | '4' | GuiMenu_DEPTH_1,
		        menu_cb_InsertIntervalOnTier4, this);
		FunctionAreaMenu_addCommand (intervalMenu, U"Add interval on tier 5",
		        GuiMenu_COMMAND | '5' | GuiMenu_DEPTH_1,
		        menu_cb_InsertIntervalOnTier5, this);
		FunctionAreaMenu_addCommand (intervalMenu, U"Add interval on tier 6",
		        GuiMenu_COMMAND | '6' | GuiMenu_DEPTH_1,
		        menu_cb_InsertIntervalOnTier6, this);
		FunctionAreaMenu_addCommand (intervalMenu, U"Add interval on tier 7",
		        GuiMenu_COMMAND | '7' | GuiMenu_DEPTH_1,
		        menu_cb_InsertIntervalOnTier7, this);
		FunctionAreaMenu_addCommand (intervalMenu, U"Add interval on tier 8",
		        GuiMenu_COMMAND | '8' | GuiMenu_DEPTH_1,
		        menu_cb_InsertIntervalOnTier8, this);
	}

	FunctionAreaMenu_addCommand (
	        intervalMenu, U"- Query interval:", 0, nullptr, this);
	FunctionAreaMenu_addCommand (intervalMenu,
	        U"Get starting point of interval", 1,
	...
	EditorMenu boundaryMenu =
	        Editor_addMenu (our functionEditor (), U"Boundary", 0);
	/*FunctionAreaMenu_addCommand (boundaryMenu, U"Move to B", 0,
	menu_cb_MoveToB, this); FunctionAreaMenu_addCommand (boundaryMenu, U"Move to
	E", 0, menu_cb_MoveToE, this);*/
	if (our editable ()) {
		FunctionAreaMenu_addCommand (
		        boundaryMenu, U"New boundary or point:", 0, nullptr, this);
		FunctionAreaMenu_addCommand (boundaryMenu, U"Add on selected tier",
		        GuiMenu_ENTER | GuiMenu_DEPTH_1, menu_cb_InsertOnSelectedTier,
		        this);
		FunctionAreaMenu_addCommand (boundaryMenu, U"Add on tier 1",
		        GuiMenu_COMMAND | GuiMenu_F1 | GuiMenu_DEPTH_1,
		        menu_cb_InsertOnTier1, this);
		FunctionAreaMenu_addCommand (boundaryMenu, U"Add on tier 2",
		        GuiMenu_COMMAND | GuiMenu_F2 | GuiMenu_DEPTH_1,
		        menu_cb_InsertOnTier2, this);
		FunctionAreaMenu_addCommand (boundaryMenu, U"Add on tier 3",
		        GuiMenu_COMMAND | GuiMenu_F3 | GuiMenu_DEPTH_1,
		        menu_cb_InsertOnTier3, this);
		FunctionAreaMenu_addCommand (boundaryMenu, U"Add on tier 4",
		        GuiMenu_COMMAND | GuiMenu_F4 | GuiMenu_DEPTH_1,
		        menu_cb_InsertOnTier4, this);
		FunctionAreaMenu_addCommand (boundaryMenu, U"Add on tier 5",
		        GuiMenu_COMMAND | GuiMenu_F5 | GuiMenu_DEPTH_1,
		        menu_cb_InsertOnTier5, this);
		FunctionAreaMenu_addCommand (boundaryMenu, U"Add on tier 6",
		        GuiMenu_COMMAND | GuiMenu_F6 | GuiMenu_DEPTH_1,
		        menu_cb_InsertOnTier6, this);
		FunctionAreaMenu_addCommand (boundaryMenu, U"Add on tier 7",
		        GuiMenu_COMMAND | GuiMenu_F7 | GuiMenu_DEPTH_1,
		        menu_cb_InsertOnTier7, this);
		FunctionAreaMenu_addCommand (boundaryMenu, U"Add on tier 8",
		        GuiMenu_COMMAND | GuiMenu_F8 | GuiMenu_DEPTH_1,
		        menu_cb_InsertOnTier8, this);
		FunctionAreaMenu_addCommand (boundaryMenu, U"Add on all tiers",
		        GuiMenu_COMMAND | GuiMenu_F9 | GuiMenu_DEPTH_1,
		        menu_cb_InsertOnAllTiers, this);
		FunctionAreaMenu_addCommand (
		        boundaryMenu, U"- Modify boundary or point:", 0, nullptr, this);
		if (our borrowedSoundArea &&
		        !Thing_isa (our borrowedSoundArea, classLongSoundArea)) {
			FunctionAreaMenu_addCommand (boundaryMenu,
			        U"Move to nearest zero crossing", 1, menu_cb_MoveToZero,
			        this);
		}
		FunctionAreaMenu_addCommand (boundaryMenu, U"Remove",
		        GuiMenu_OPTION | GuiMenu_BACKSPACE | GuiMenu_DEPTH_1,
	...
	if (our editable ()) {
		...
		FunctionAreaMenu_addCommand (
		        tierMenu, U"Add point tier...", 1, menu_cb_AddPointTier, this);
		FunctionAreaMenu_addCommand (
		        tierMenu, U"Duplicate tier...", 1, menu_cb_DuplicateTier, this);
		FunctionAreaMenu_addCommand (
		        tierMenu, U"- Modify tier:", 0, nullptr, this);
		FunctionAreaMenu_addCommand (
		        tierMenu, U"Rename tier...", 1, menu_cb_RenameTier, this);
		FunctionAreaMenu_addCommand (tierMenu, U"Remove all text from tier", 1,
		        menu_cb_RemoveAllTextFromTier, this);
		FunctionAreaMenu_addCommand (
		        tierMenu, U"Remove entire tier", 1, menu_cb_RemoveTier, this);
	}
	FunctionAreaMenu_addCommand (
	        tierMenu, U"- Extract to list of objects:", 0, nullptr, this);
	...
	if (our functionEditor ()->textArea)
		GuiText_setChangedCallback (
		        our functionEditor ()->textArea, gui_text_cb_changed, this);

	if (our spellingChecker) {
		EditorMenu spellMenu =
		        Editor_addMenu (our functionEditor (), U"Spell", 0);
		FunctionAreaMenu_addCommand (spellMenu, U"Check spelling in tier",
		        GuiMenu_COMMAND | GuiMenu_OPTION | 'L', menu_cb_CheckSpelling,
		        this);
		FunctionAreaMenu_addCommand (spellMenu, U"Check spelling in interval",
		        0, menu_cb_CheckSpellingInInterval, this);
		FunctionAreaMenu_addCommand (
		        spellMenu, U"-- edit lexicon --", 0, nullptr, this);
		FunctionAreaMenu_addCommand (spellMenu,
		        U"Add selected word to user dictionary", 0,
		        menu_cb_AddToUserDictionary, this);
	}

	EditorMenu boundaryLinesMenu =
	        Editor_addMenu (our functionEditor (), U"BoundaryLines", 0);
	if (our editable ()) {
		our boundaryLinesToggle = FunctionAreaMenu_addCommand (
		        boundaryLinesMenu, U"Show All Boundary Lines",
		        GuiMenu_CHECKBUTTON | (instancePref_allBoundaryLines_show ()
		                                              ? GuiMenu_TOGGLE_ON
		                                              : 0),
	...
	if (our borrowedSoundAnalysisArea) {
		EditorMenu tierNotesMenu =
		        Editor_addMenu (our functionEditor (), U"TierNotes", 0);
		our tierNotesToggle = FunctionAreaMenu_addCommand (tierNotesMenu,
		        U"Show All Tier Notes",
		        GuiMenu_CHECKBUTTON |
		                (instancePref_allTierNotes_show () ? GuiMenu_TOGGLE_ON
		                                                   : 0),
		        menu_cb_showAllTierNotes, this);

		// Separator before frequency resolution submenu
		FunctionAreaMenu_addCommand (tierNotesMenu,
		        U"-- tier notes frequency resolution --", 0, nullptr, this);

		// Frequency resolution options (radio buttons)
		bool is005 = (instancePref_tierNotesFrequencyResolution () == 0.05);
		our tierNotesResolution005Toggle = FunctionAreaMenu_addCommand (
		        tierNotesMenu, U"Tier Notes Frequency Resolution: 0.05",
		        GuiMenu_RADIO_FIRST | (is005 ? GuiMenu_TOGGLE_ON : 0),
		        menu_cb_tierNotesFrequencyResolution005, this);
		our tierNotesResolution025Toggle = FunctionAreaMenu_addCommand (
		        tierNotesMenu, U"Tier Notes Frequency Resolution: 0.25",
		        GuiMenu_RADIO_NEXT | (!is005 ? GuiMenu_TOGGLE_ON : 0),
		        menu_cb_tierNotesFrequencyResolution025, this);

		// Initialize the frequency resolution in SoundAnalysisArea
		our borrowedSoundAnalysisArea->tierNotesFrequencyResolution =
		        instancePref_tierNotesFrequencyResolution ();

		our borrowedSoundAnalysisArea->setClickToChangePitchCallback (
		        soundAnalysisArea_cb_clickToChangePitch, this);
	}

	if (our borrowedSoundArea && our borrowedSoundAnalysisArea) {
		EditorMenu viewMenu =
		        Editor_addMenu (our functionEditor (), U"View", 0);
		FunctionAreaMenu_addCommand (viewMenu, U"View proportion...", 0,
		        menu_cb_viewProportion, this);
	}
}
void structTextGridArea ::v_updateMenuItems () {
	TextGridArea_Parent ::v_updateMenuItems ();
	GuiThing_setSensitive (extractSelectedTextGridPreserveTimesButton,
	        our endSelection () > our startSelection ());
	GuiThing_setSensitive (extractSelectedTextGridTimeFromZeroButton,
	        our endSelection () > our startSelection ());
}

void structTextGridArea ::v_updateText () {
	if (!our textGrid ()) return;   // BUG: should not be needed
	if (our suppressTextCursorJump) return;
	...
	if (our selectedTier != 0) {
		...
		AnyTextGridTier_identifyClass (
		        our textGrid ()->tiers->at[our selectedTier], &intervalTier,
		        &textTier);
		if (intervalTier) {
			const integer iinterval = IntervalTier_timeToIndex (
			        intervalTier, our startSelection ());
			if (iinterval) {
				TextInterval interval = intervalTier->intervals.at[iinterval];
				if (interval->text) newText = interval->text.get ();
			}
		} else {
			Melder_assert (isdefined (our startSelection ()));
			const integer ipoint = AnyTier_hasPoint (
			        textTier->asAnyTier (), our startSelection ());
			if (ipoint) {
				TextPoint point = textTier->points.at[ipoint];
				if (point->mark) newText = point->mark.get ();
			}
		}
	}
	if (our functionEditor ()->textArea) {
		our suppressRedraw =
		        true;   // prevent valueChangedCallback from redrawing
...
void TextGridArea_drawSelectionViewer (TextGridArea me) {
	...
	const double cellWidth_points =
	        Graphics_dxWCtoMM (my graphics (), 1.0) * pointsPerMillimetre;
	const double cellHeight_points =
	        Graphics_dyWCtoMM (my graphics (), 1.0) * pointsPerMillimetre;
	const double fontSize =
	        std::min (0.8 * cellHeight_points, 1.2 * cellWidth_points);
	Graphics_setFontSize (my graphics (), fontSize);
	Graphics_setTextAlignment (my graphics (), Graphics_CENTRE, Graphics_HALF);
	for (integer irow = 1; irow <= 12; irow++)
		for (integer icol = 1; icol <= 10; icol++)
			Graphics_text (my graphics (), 0.0 + 1.0 * icol, 13.0 - 1.0 * irow,
...
void TextGridArea_clickSelectionViewer (
	...
	if (my functionEditor ()->textArea) {
		...
		autostring32 oldText = GuiText_getStringAndSelectionPosition (
		        my functionEditor ()->textArea, &first, &last);
		...
		if (my selectedTier != 0) {
			...
			if (intervalTier) {
				...
				if (selectedInterval != 0) {
					...
					my suppressRedraw = true;   // prevent valueChangedCallback
					                            // from redrawing
					trace (U"setting new text ", newText.string);
					GuiText_setString (
					        my functionEditor ()->textArea, newText.string);
					GuiText_setSelection (my functionEditor ()->textArea,
					        first + 1, first + 1);
				...
				if (selectedPoint != 0) {
					...
					point->mark.reset ();
					if (Melder_findInk (
					            newText.string))   // any visible characters?
